...
from dags.dataproc.monitoring.cluster_monitoring import ClusterDetails, cluster_groupby, add_duplicate_clusters, \
    ...
    CreatorDetails, SlackThreadPoster, assume_role_for_dev
...
class TestGetMaxPermittedDuration(unittest.TestCase):
    ...
    def test_no_matching_tags_returns_default(self):
        ...
        self.assertEqual(DurationExceeded.get_max_permitted_duration(tags), ClusterDurations.DEFAULT_MAX_DURATION)
    ...
    def test_duration_tag_returns_correct_limit(self):
        ...
        self.assertEqual(DurationExceeded.get_max_permitted_duration(tags), ClusterDurations.DURATION_VALUES["long"])
    ...
    def test_multiple_matching_tags_returns_most_permissive_value(self):
        ...
        self.assertEqual(DurationExceeded.get_max_permitted_duration(tags), ClusterDurations.DURATION_VALUES["very_long"])
...
class TestDevRoleAssumption(unittest.TestCase):

    @patch("dags.dataproc.monitoring.cluster_monitoring.AwsBaseHook")
    def test_assume_role_for_dev(self, mock_aws_base_hook):
        mock_hook_instance = MagicMock()
        mock_aws_base_hook.return_value = mock_hook_instance

        mock_emr_client = MagicMock()
        mock_hook_instance.get_conn.return_value = mock_emr_client

        region_name = "us-east-1"
        result_client = assume_role_for_dev(region_name)

        mock_aws_base_hook.assert_called_with(aws_conn_id="aws_dev", client_type="emr", region_name=region_name)
        mock_hook_instance.get_conn.assert_called_once()
        self.assertEqual(result_client, mock_emr_client)


class TestDevSlackMessaging(unittest.TestCase):

    def test_slack_message_for_termination_protection(self):
        mock_slack_client = MagicMock()

        cluster_with_protection = ClusterDetails(
            cluster_id="cl-termination-protected",
            name="protection-test-cluster",
            creation_date_time=datetime.now(tz=tzlocal()),
            cloud_provider=AwsCloudProvider(),
            region_name="us-east-1",
            tags={
                "Environment": "Prod",
                "ClusterName": "cl-termination-protected"
            },
            termination_protected=True,
        )

        poster = SlackThreadPoster(mock_slack_client, cluster_with_protection, is_deletion_message=True, account="dev")
        poster._post_cluster_thread_message(thread_ts="12345")

        message_text = mock_slack_client.post_message.call_args[1]["text"]
        self.assertIn("termination-protected", message_text)
        self.assertIn("will be removed to proceed with deletion", message_text)
