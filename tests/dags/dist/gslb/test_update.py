...
import unittest
from dags.dist.gslb.ns1 import Answer, NS1Client, Record, NotFoundException
from dags.dist.gslb.update import UpdateTask
from dags.dist.gslb.prometheus import MatrixQueryResponse, MatrixResult, PrometheusClient, VectorQueryResponse, VectorData, VectorResult
...
class PrometheusClientFake(PrometheusClient):
    ...
    def __init__(self, query_responses: list[tuple[re.Pattern, Timeline]]):
        super().__init__()
...
class UpdateTestCase(unittest.TestCase):
    ...
    def test_normal_case(self):
        now = time()
        timeline = Timeline([weights_response(now, {'ca2': 1_000_000, 'ca4': 2_000_000})])
        ...
        task = UpdateTask(test_config, prometheus_client, ns1_client)
    ...
    def test_negative_weights(self):
        now = time()
        timeline = Timeline([weights_response(now, {'ca2': -1, 'ca4': 2})])
        prometheus_client = PrometheusClientFake([(re.compile('bid_request_count'), timeline)])
        ns1_client = NS1ClientFake()
        ns1_client.create_record(domain_name, default_record())
        task = UpdateTask(test_config, prometheus_client, ns1_client)
    ...
    def test_lookback(self):
        ...
        timeline = Timeline([weights_response(now - delta, {'ca2': 1_000_000, 'ca4': 2_000_000})])
        prometheus_client = PrometheusClientFake([(re.compile('bid_request_count'), timeline)])
        ns1_client = NS1ClientFake()
        ns1_client.create_record(domain_name, default_record())
        task = UpdateTask(test_config, prometheus_client, ns1_client)
    ...
    def test_no_huge_datacenter(self):
        now = time()
        timeline = Timeline([weights_response(now, {'ca2': 1, 'ca4': 2_000_000})])
        prometheus_client = PrometheusClientFake([(re.compile('bid_request_count'), timeline)])
        ns1_client = NS1ClientFake()
        ns1_client.create_record(domain_name, default_record())
        task = UpdateTask(test_config, prometheus_client, ns1_client)
