...
class DurationExceeded(AlarmReason):
    one_day = 60 * 60 * 24
    default_max_duration = 0.75 * one_day
    duration_values = {
        "extremely_long": 10 * one_day,
        "very_long": 5 * one_day,
        "long": 1.5 * one_day,
    }
    legacy_duration_tags = {"very_long_running": 5 * one_day, "long_running": 1.5 * one_day}
...
""",
    ...
    def get_max_permitted_duration(cls, tags: Dict[str, str]) -> float:
        if extension := tags.get(duration_tag):
            return cls.duration_values.get(extension, cls.default_max_duration)
        else:
            for key in cls.legacy_duration_tags.keys():
                if key in tags:
                    return cls.legacy_duration_tags[key]
            return cls.default_max_duration
...
class DuplicateClusterDetected(AlarmReason):
    ...
    def __init__(self, cluster: "ClusterDetails", duplicates: List["ClusterDetails"]):
        ...
        message = f"Duplicate clusters: *<{cluster.link}|{cluster.name}>*, {duplicate_links}"
...
class UserNotInTeam(AlarmReason):
    ...
    def __init__(self, user: str, team: str, link: str, cluster_id: str) -> None:
        super().__init__(
            slack_message=f"""This cluster (*<{link}|{cluster_id}>*) has been created!""",
    ...
    def check(cls, cluster: "ClusterDetails") -> Optional["AlarmReason"]:
        ...
        if not is_user_in_team(creator, cluster.team):
            return cls(creator, cluster.team, cluster.link, cluster.cluster_id)
        return None
...
class SlackThreadPoster:
    ...
    def __init__(self, client: TtdSlackClient, cluster: ClusterDetails, is_deletion_message: bool):
        self._client = client
...
def send_slack_alarms(clusters_to_delete: list[ClusterDetails], clusters_to_warn: list[ClusterDetails]) -> None:
    api_token = BaseHook.get_connection("slack").password
    slack_client = TtdSlackClient(api_token=api_token)
    for cluster in clusters_to_delete:
        deletion_thread_poster = SlackThreadPoster(slack_client, cluster, True)
        deletion_thread_poster.post()

    for cluster in clusters_to_warn:
        warning_thread_poster = SlackThreadPoster(slack_client, cluster, False)
        warning_thread_poster.post()


def get_client(cloud_provider: CloudProvider, region_name: str) -> Union[BaseClient, AliEMRHook]:
...
def get_active_cluster_details(client: Union[BaseClient, AliEMRHook], region_name: str) -> List[ClusterDetails]:
...
def check_clusters(cloud_provider: CloudProvider, region_name: str) -> None:
    client = get_client(cloud_provider=cloud_provider, region_name=region_name)
    active_cluster_details = get_active_cluster_details(client=client, region_name=region_name)
    ...
    send_slack_alarms(clusters_for_deletion, clusters_for_warnings)
...
def create_dag(job_name: str, cloud_provider: CloudProvider, regions: list[str]) -> TtdDag:
    ttd_dag = TtdDag(
        dag_id=job_name,
    ...
    for region in regions:
        check_clusters_task = OpTask(
            op=PythonOperator(
                task_id=f"check_clusters_{region}",
        ...
        ttd_dag >> check_clusters_task

    return ttd_dag


dag_info = [
    ("emr-cluster-monitoring", CloudProviders.aws, ["us-east-1"]),
    ("alicloud-emr-cluster-monitoring", CloudProviders.ali, ["cn-shanghai"]),
]

for (dag_id, cloud, regions) in dag_info:
    dag = create_dag(dag_id, cloud, regions)
