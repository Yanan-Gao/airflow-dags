...
class ACRExposureSegmentGeneration:
    ...
    def get_task(
        ...
        sub_folder = config.provider + "-" + config.country + '-exposure'
        java_options_list = [
            ("date", config.run_date),
            (
                "acrProviderEnrichedPath", SegmentDatasources(
                    config.provider
                ).get_frequency_dataset(config.country, 90, config.frequency_aggregation_version).get_root_path()
        ...
        cluster = EmrClusterTask(
            name=f'exposure_segment_generation_{config.provider}_{config.country}',
        ...
        cluster.add_parallel_body_task(
            EmrJobTask(
                ...
                name='ACRExposureSegmentGeneration',
                class_name="jobs.ctv.linear.acr.segment.tdidmapping.ExposureSegmentGenerator",
        ...
        return ChainOfTasks(
            task_id="exposure_segment_generation", tasks=[check_graph_recency, cluster]
        ).as_taskgroup("exposure_segment_generation")
