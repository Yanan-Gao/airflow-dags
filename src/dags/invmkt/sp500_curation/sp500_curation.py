...
max_active_runs = 1
run_date = "{{ (dag_run.logical_date + macros.timedelta(days=5)).strftime(\"%Y-%m-%d\") }}"
dependency_date = "{{ (logical_date + macros.timedelta(days=5)).to_datetime_string() }}"
...
image_dev = "production.docker.adsrvr.org/metadata-airflow:0.1.790"
image_prod = "production.docker.adsrvr.org/metadata-airflow:0.1.790"
...
property_seller_metadata_wait = DatasetCheckSensor(
    ...
    timeout=60 * 60 * 8  # wait up to 5 hours
)
...
# K8S pod
####################################################################################################################
# Todo: To be enabled when batch job is ready.
...
sp500_curation_pod = TtdKubernetesPodOperator(
    ...
    name='SP500CurationPod',
    task_id="SP500CurationPod",
    dnspolicy='Default',
    get_logs=True,
...
sp500_curation_pod_task = OpTask(op=sp500_curation_pod)

###########################################
#   Dependencies
###########################################
# dag >> sp500_curation_pod_task
...
publisher_quality_metrics_job_step >> sp500_curation_pod_task
