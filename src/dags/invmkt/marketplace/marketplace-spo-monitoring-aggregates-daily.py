...
# Prod variables
job_name = 'marketplace-spo-monitoring-aggregates-daily'
cluster_name = "invmkt_marketplace_spo_monitoring"
invmkt = INVENTORY_MARKETPLACE().team
emr_release_label = AwsEmrVersions.AWS_EMR_SPARK_3_5
job_schedule_interval = timedelta(days=1)
job_start_date = datetime(2025, 2, 20, 7)
job_environment = TtdEnvFactory.get_from_system()
max_active_runs = 2

# Jar variables
jar_path = 's3://ttd-build-artefacts/eldorado/release-spark-3/main-spark-3/latest/eldorado-invmkt-assembly.jar'
class_name = 'jobs.marketplaceactivitydashboard.MarketplaceSellerSPOMonitor'
...
dag: TtdDag = TtdDag(
    ...
    tags=[invmkt.jira_team, 'marketplace-avails-metadata-aggregates'],
    default_args={
...
on_demand_weighted_capacity = 64 * 2
...
marketplace_avails_metadata_cluster_task = EmrClusterTask(
    name=cluster_name,
...
config_list = [('date', run_date), ('datasetGenerationType', 'MarketplaceMetadataGeneration')]
...
job_step = EmrJobTask(
    ...
    cluster_specs=marketplace_avails_metadata_cluster_task.cluster_specs
)

marketplace_avails_metadata_cluster_task.add_parallel_body_task(job_step)
...
dag >> marketplace_avails_metadata_cluster_task
wait_complete >> marketplace_avails_metadata_cluster_task.first_airflow_op()
