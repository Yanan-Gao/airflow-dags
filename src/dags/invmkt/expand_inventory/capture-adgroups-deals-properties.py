...
from dags.invmkt.expand_inventory.common import build_s3_destination_uri_task, get_is_prod_enviroment, BUILD_S3_DESTINATION_URI_TASK_ID, \
    build_create_version_task, build_cleanup_old_batches_task, S3_DEST_URI_BUCKET_KEY, S3_DEST_URI_PREFIX_KEY, CREATE_VERSION_TASK_ID, VERSION_ID_KEY, \
    S3_DEST_URI_KEY, build_set_version_to_complete_task, PROVISIONING_DB_NAME, EIS_DOMAIN_NAME

DAG_ID = 'capture-adgroups-deals-properties'
job_schedule_interval = timedelta(days=7)
job_start_date = datetime(2024, 3, 21, 00, 00)
INVMKT_TEAM = 'INVMKT'
INVMKT_ALARM_SLACK_CHANNEL = 'scrum-invmkt-alarms'
is_prod_environment = get_is_prod_enviroment()

capture_adgroups_deals_properties_dag = TtdDag(
    dag_id=DAG_ID,
...
dag = capture_adgroups_deals_properties_dag.airflow_dag

STORAGE_BUCKET = "ttd-inventory-recommendations-metadata"
S3_DATASET_NAME = 'recommended_properties'
...
# First EMR version that supports Spark 3.2.1 which is used by the expand-inventory-selection package.
EMR_RELEASE = 'emr-6.7.0'
cluster_task = EmrClusterTask(
    name='capture_adgroups_deals_properties_cluster',
...
capture_and_persist_adgroups_deals_properties_to_s3_task = EmrJobTask(
    name="capture_and_persist_adgroups_deals_properties_to_s3_task",
    class_name="com.thetradedesk.ds.libs.expandinventoryselection.spark.CaptureAdGroupsDealsPropertiesJob",
    executable_path=
...
cluster_task.add_parallel_body_task(capture_and_persist_adgroups_deals_properties_to_s3_task)

RECOMMENDED_PROPS_BATCH_TYPE_ID = 2
create_version_task = build_create_version_task(RECOMMENDED_PROPS_BATCH_TYPE_ID)
"""
...
PERSISTENCE_TASK_NAME = "persist_adgroups_deals_properties_provisioning"

DATASET_NAME = "recommended_properties"
PERSISTENCE_STAGING_BATCH_SIZE = 50_000
PERSISTENCE_SLEEP_BETWEEN_BATCHES_IN_SECONDS = 1
PERSISTENCE_PROCESS_TIMEOUT_IN_SECONDS = 12 * 60 * 60
persist_adgroups_deals_properties_provisioning_task = create_cloud_storage_to_sql_db_task(
    ...
    destination_table_name="InventorySelectionAdgroupProperties",
    column_mapping=[
        CloudStorageToSqlColumn("AdGroupId", "AdGroupId"),
        CloudStorageToSqlColumn("PropertyIdString", "PropertyId"),
        CloudStorageToSqlColumn("AvailsCount", "AvailsCount"),
...
set_version_to_complete_task = build_set_version_to_complete_task(RECOMMENDED_PROPS_BATCH_TYPE_ID)

CLEANUP_SPROC_NAME = 'prc_DeleteFromInventorySelectionAdgroupProperties'
...
capture_adgroups_deals_properties_dag >> OpTask(op=build_s3_destination_uri_task) >> OpTask(op=create_version_task) >> cluster_task

capture_and_persist_adgroups_deals_properties_to_s3_task >> persist_adgroups_deals_properties_provisioning_task >> OpTask(
