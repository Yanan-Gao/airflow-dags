...
def get_retention_start_date(enable_date, start_date, retention_period):
    feed_production_start_date = max(enable_date, start_date)
    return feed_production_start_date + timedelta(days=(retention_period + 1))


def get_latest_date_to_delete(current_date, retention_period):
    return current_date - timedelta(days=(retention_period + 1))


def get_dates_to_delete(current_date, enable_date, start_date, retention_period):
    """
    Determine the range of dates to delete based on retention policy.

    For backfill feeds, on the first day that retention starts, return the range of dates to delete from start date
    to enable date.
    Return None if nothing needs to be deleted.

    Returns:
    A tuple containing the start and end dates for deletion, or None if no dates need to be deleted.
    """
    retention_start_date = get_retention_start_date(enable_date, start_date, retention_period)
    if current_date < retention_start_date:
        return None

    latest_date_to_delete = get_latest_date_to_delete(current_date, retention_period)
    if current_date == retention_start_date and start_date < enable_date:
        return start_date, latest_date_to_delete

    return latest_date_to_delete, latest_date_to_delete
