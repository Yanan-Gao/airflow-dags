...
# conversionTracker in REDS consist of 2 data types: conversionTracker (4) and lateConversionTracker (236)
lwdb_conversion_tracker_counts_query = jinja2.Template(
    """
    select {{ snowflake_gating_id }}, {{ task_id }}, '{{ start_time }}', '{{ end_time }}', coalesce(sum(f.SuccessfulLineCount),0) as totalCount from dbo.LogFile f where f.LogStartTime >= '{{ start_time }}' and f.LogStartTime < '{{ end_time }}' and f.LogTypeId in (4, 236) and f.CloudServiceId = dbo.fn_Enum_CloudService_AWS()
    """
)
...
def get_snowflake_lwdb_counts(prefix, dry_run):
    ...
    for item in hourstocompare:
        ...
        if str(item[2]) == "4":
            lwdbQuery = lwdbQuery + lwdb_conversion_tracker_counts_query.render(
                snowflake_gating_id=item[0], task_id=item[1], start_time=item[3], end_time=item[4]
            )
        else:
