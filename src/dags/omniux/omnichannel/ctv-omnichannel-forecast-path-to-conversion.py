...
dag_id = "ctv-omnichannel-forecast-path-to-conversion"
...
# {"start_date": "2024-06-29", "end_date_impressions": "2024-07-28", "attribution_window": "60", "run_datetime": "2024-10-02T12:01:02", "use_legacy_channel": "false"}
...
use_legacy_channel_key = "use_legacy_channel"
group_only_key = "group_only"

ttd_ctv_bucket = "ttd-ctv"
java_options = [
    # ("advertiserIds", "hcx64wq,zo3cxwj"),
    ("rootForecastPathToConversionPath", f"s3a://{ttd_ctv_bucket}"),
    ("runDateTime", xcom_pull_expr(run_date_time_key)),
    ("groupOnly", xcom_pull_expr(group_only_key)),
    ...
    ("minImpressionCount", "4000"),
    ("minForecastInputCount", "5"),
    ("writeForecastAvgPathSummaryDataset", "true"),
    ("useLegacyChannel", xcom_pull_expr(use_legacy_channel_key)),
    ("ttd.ds.ForecastPathSummaryDataset.isInChain", "true"),  # Used for post-processing job
    ("pipeline", "Forecast"),
...
def get_emr_task_args(**context):
    ...
    defaults = {
        ...
        use_legacy_channel_key: "false",
        group_only_key: "false"
    }
...
check_latest_bfs_aes_exist = OpTask(
    op=DatasetCheckSensor(
        ...
        datasets=[
            ...
            Datasources.ctv.bidfeedback_daily_subset_no_partition(env.dataset_read_env)
        ],
...
def ExecuteOnDemandDataMoveWrapper(**context):
    ...
    op_kwargs = {
        ...
        'sproc_arguments': {
            'taskId': 1000586,  # dbo.fn_Enum_Task_OnDemandImportOmnichannelForecastPathToConversion()
...
logworkflow_open_sql_import_gate = OpTask(
    op=PythonOperator(
        ...
        task_id="logworkflow_forecast_ptc_sql_import_on_demand",
        trigger_rule=TriggerRule.ONE_SUCCESS,
        provide_context=True,
    )
)

job_package = ("com.thetradedesk.ctv.upstreaminsights.pipelines.omnichannelview.forecastptc")
...
post_processing_cluster = emr_task("post-processing", f"{job_package}.ForecastPostProcessingJob", 1, master_instance_type=M5d.m5d_2xlarge())
...
merge_task_cluster >> post_processing_cluster >> logworkflow_open_sql_import_gate
