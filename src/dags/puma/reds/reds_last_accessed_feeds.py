...
dag = TtdDag(
    dag_id=job_name,
    start_date=datetime(2025, 6, 1, 0, 0, 0, 0),
...
async def store_sumo_logs(execution_datetime) -> Dict[str, SumoData]:
    ...
    query = rf"""
        ...
        | json field=_raw "resources[1].ARN" as arn
...
def list_reds_regex_paths_feedid() -> List[Tuple[str, str]]:
    ...
    filepaths = [(feed.destination_path_regex, feed.feed_id) for feed in feeds]
    return filepaths
...
async def build_trie() -> FilePathTrie:
    ...
    for destination_path_regex, feed_id in list_reds_regex_paths_feedid():
...
def check_s3_metadata(filepath):
    try:
        ...
        if 'Metadata' in response:
            ...
            workflow_type = metadata.get('workflow-type')
            if feed_id:
                logging.info(f"Found feedid: {feed_id} and workflow_type {workflow_type} from metadata!")
                if workflow_type:
                    workflow_type = workflow_type.lower()
                    if "reds" in workflow_type:
                        workflow_type_int = 1
                    elif "exposure" in workflow_type:
                        workflow_type_int = 2
                return int(feed_id), workflow_type_int
...
def validate_feedid_and_update_dict(feed_id, feed_id_to_data, sumo_data, db_feed_ids):
    ...
    if feed_id not in db_feed_ids:
        feed_id, workflow_type = check_s3_metadata(sumo_data.filepath)
        sumo_data.feed_id = feed_id
        sumo_data.workflow_type = workflow_type
...
def find_feed_id_from_filepath(trie: FilePathTrie, sumo_logs: Dict[str, SumoData]) -> Tuple[Dict[int, SumoData], bool]:
    ...
    for filepath, sumo_data in sumo_logs.items():
        try:
            ...
            if feed_id is not None:
                ...
                result = check_s3_metadata(filepath)
                if result is not None:
                    ...
                    fail_matches += 1
                    logging.error(f"Could not find feed_id using regex, trie, or S3 for filepath: {filepath}")
        except Exception as e:
            caught_exception_count += 1
            logging.error(f"Ran into an exception while parsing feeds for filepath : {filepath} with exception {e}")
    ...
    logging.info(f"Number of Caught Exceptions: {caught_exception_count}")
    end_time = time.time()
    ...
    filepath_caught_exceptions: Gauge = get_or_register_gauge(
        job_name, f"{job_name}_filepath_caught_exceptions_{version}",
        f"{job_description} - Caught Exceptions - we ran into an unexpected issue when processing a feed. Manual action is required", []
    )
    filepath_caught_exceptions.set(caught_exception_count)
    logging.info(f"Execution time of Filepath/FeedId Comparison: {(end_time - start_time) / 60} min.")
    should_fail_task = caught_exception_count > 0
    return feed_id_to_data, should_fail_task
...
async def async_update_last_accessed_task(execution_datetime):
    ...
    feedid_to_datetime_user, should_fail_job = find_feed_id_from_filepath(trie, sumo_logs)
    update_last_accessed_table(feedid_to_datetime_user)
    if should_fail_job:
        raise Exception("Job failed due to errors during feed ID parsing.")
