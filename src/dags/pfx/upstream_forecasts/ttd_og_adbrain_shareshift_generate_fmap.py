...
s3_root_path_og = "s3a://ttd-ctv/upstream-forecast/data-preparation-og-adbrain"
...
dag = TtdDag(
    dag_id="ttd-og-adbrain-shareshift-generate-fmap",
...
cluster_name = "ttd-og-adbrain-shareshift-generate-fmap-cluster"
...
forecast_config_job_task = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=[("ttd.env", TTD_ENV), ("targetDate", target_date), ("s3RootPath", s3_root_path_og),
                                       ("useShareshift", "true")],
...
rails_mapping_job_task = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=[
        ("ttd.env", TTD_ENV),
        ("graphName", "openGraphAdBrain"),  # This will load open graph avails
...
frequency_map_generation_job_task = EmrJobTask(
    ...
    executable_path=JAR_PATH,
    eldorado_config_option_pairs_list=[("ttd.env", TTD_ENV), ("targetDate", target_date),
                                       ("overallSamplingFactor", overall_sampling_factor), ("s3RootPath", s3_root_path_og),
                                       ("useShareshift", "true")],
    cluster_specs=cluster_task.cluster_specs,
    configure_cluster_automatically=False,
    additional_args_option_pairs_list=additional_args_option_pairs_list,
...
cluster_task.add_parallel_body_task(
    ChainOfTasks("ttd-adbrain-shareshift-generate-fmap-tasks", [forecast_config_job_task, frequency_map_generation_job_task])
)
