...
validation_dag_id = "ctv-upstream-forecast-validation-v3"
...
def get_closest_previous_date(target_date: str) -> Any | None:
    most_recent_date = Datasources.ctv.upstream_forecast_validation_pass_rate_v3.check_recent_data_exist(
    ...
    if most_recent_date.is_success:
        ...
        logging.info(f'Found most recent date for v3 pipeline {base_date_str}')
        return base_date_str
    logging.info(f'Did not find any previous date for target date {target_date} in v3 pipeline')
    return None
