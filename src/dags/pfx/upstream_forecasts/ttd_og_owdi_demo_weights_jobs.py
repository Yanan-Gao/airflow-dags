...
OWDI_DEMO_WEIGHT_JOB_CLASS = "com.thetradedesk.etlforecastjobs.upstreamforecasting.datapreparation.DemoWeightsFromOwdiJob"
...
owdi_demo_dag_id = "ttd-og-owdi-demo-weights-job-dag"
owdi_calculate_dates_task_id = "og-owdi-calculate-dates-task"

# dates used for down stream pipeline
target_date_key = "target-date"
demo_target_date_key = "demo-target-date"

job_start_date = datetime(2025, 2, 5, 13)  # Wednesday 1pm utc, 5am local,
job_end_date = None
...
cluster_task = EmrClusterTask(
    name="ttd-og-owdi-demo-weights-gen-cluster",
...
generate_owdi_demo_weights_emr_step_task = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=[
        ...
        ("owdiPath", "s3://ttd-datprd-us-east-1/application/radar/data/export/general/Iav2PersonDemographic"),
        ("s3RootPath", "s3://ttd-ctv/upstream-forecast/data-preparation-og"),  # These two are additional for open graph version
    ],
...
generate_owdi_max_counts_emr_step_task = EmrJobTask(
    name="owdi-max-counts-gen-step",
    class_name=OWDI_MAX_COUNTS_JOB_CLASS,
    executable_path=JAR_PATH,
    eldorado_config_option_pairs_list=[
        ("ttd.env", "prod"),
        ("demoDate", xcom_template_to_get_value(owdi_date_key_2)),
        ("targetDate", xcom_template_to_get_value(owdi_date_key_2)),
        ("s3RootPath", "s3://ttd-ctv/upstream-forecast/data-preparation-og"),  # for open graph
    ],
    cluster_specs=cluster_task.cluster_specs,
    configure_cluster_automatically=False,
    additional_args_option_pairs_list=additional_args_option_pairs_list,
...
cluster_task.add_sequential_body_task(generate_owdi_max_counts_emr_step_task)
...
def calculate_owdi_dates(**context):
    ...
    most_recent_owdi_date1 = Datasources.ctv.open_graph_iav2_owdi_data.check_recent_data_exist(
        cloud_storage=cloud_storage, ds_date=run_date - timedelta(days=14), max_lookback=14
    ).get()
    if most_recent_owdi_date1 is None:
        raise ValueError(f'did not find first owdi date based on date {run_date.strftime("%Y-%m-%d")}')
    logging.info(f'Using iav2 owdi date 1 {most_recent_owdi_date1.strftime("%Y-%m-%d")}')
    task_instance.xcom_push(key=owdi_date_key_1, value=most_recent_owdi_date1.strftime("%Y-%m-%d"))

    most_recent_owdi_date2 = Datasources.ctv.open_graph_iav2_owdi_data.check_recent_data_exist(
        cloud_storage=cloud_storage, ds_date=run_date, max_lookback=14
    ).get()
    if most_recent_owdi_date2 is None:
        raise ValueError(f'did not find second owdi date based on date {run_date.strftime("%Y-%m-%d")}')
    logging.info(f'Using iav2 owdi date 2 {most_recent_owdi_date2.strftime("%Y-%m-%d")}')
...
trigger_fmap_task = OpTask(
    op=TriggerDagRunOperator(
        ...
        trigger_dag_id="ttd-og-iav2-generate-fmap",
        conf={
