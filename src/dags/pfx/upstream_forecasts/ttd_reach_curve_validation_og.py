...
validation_dag_id = "ctv-upstream-forecast-validation-og"
...
functional_validation = get_validation_pipeline_task(
    ...
    timeout=2,
    env=ttd_env,
    reach_curve_version=reach_curve_version,
    jar_path=jar_path
)
...
def get_closest_previous_date(target_date: str) -> Any | None:
    most_recent_date = Datasources.ctv.upstream_forecast_validation_pass_rate_og.check_recent_data_exist(
    ...
    if most_recent_date.is_success:
        ...
        logging.info(f'Found most recent date for og pipeline {base_date_str}')
        return base_date_str
    logging.info(f'Did not find any previous date for target date {target_date} in og pipeline')
    return None
...
validation_pipeline_dag_pipeline >> store_date_op >> validation_cluster >> gating_cluster
