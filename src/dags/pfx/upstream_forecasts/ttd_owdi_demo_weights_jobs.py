...
# JAR_PATH = "s3://ttd-ctv/test/shawn.tang/lib/etl-forecast-jobs.jar"

NUM_CORE_UNITS = 240

owdi_date_key_1 = 'owdi_date_1'
owdi_date_key_2 = 'owdi_date_2'  # owdi_date_2 is latest, owdi_date_1 is roughly two weeks apart
owdi_demo_dag_id = "ttd-owdi-demo-weights-job-dag"
owdi_calculate_dates_task_id = "owdi-calculate-dates-task"

# dates used for down stream pipeline
target_date_key = "target-date"
demo_target_date_key = "demo-target-date"

job_start_date = datetime(2024, 7, 31, 13)  # Wednesday 1pm utc, 5am local,
job_end_date = None
...
cluster_task = EmrClusterTask(
    name="ttd-owdi-demo-weights-gen-cluster",
...
def calculate_owdi_dates(**context):
    ...
    most_recent_owdi_date1 = Datasources.ctv.legacy_iav2_owdi_data.check_recent_data_exist(
    ...
    most_recent_owdi_date2 = Datasources.ctv.legacy_iav2_owdi_data.check_recent_data_exist(
...
trigger_fmap_task = OpTask(
    op=TriggerDagRunOperator(
        ...
        task_id='trigger-downstream-fmap-gen',
        trigger_dag_id="ttd-iav2-generate-fmap",
        conf={
