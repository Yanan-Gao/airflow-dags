...
JOB_TRANSFORMER_PUBLISHER = "publisher"
JOB_TRANSFORMER_DEAL = "deal"

source_types = [JOB_TRANSFORMER_PUBLISHER, JOB_TRANSFORMER_DEAL]
gating_type_ids = {JOB_TRANSFORMER_PUBLISHER: 2000113, JOB_TRANSFORMER_DEAL: 2000114}
source_type_data_set_name = {
    JOB_TRANSFORMER_PUBLISHER: "OpenPathPublisherAggHourlyDataSet",
    JOB_TRANSFORMER_DEAL: "OpenPathDealAggHourlyDataSet"
}

produced_datasets_list = [
    AvailsDatasources.publisher_agg_hourly_dataset.with_check_type("hour").with_region("us-east-1"),
    AvailsDatasources.deal_agg_hourly_dataset.with_check_type("hour").with_region("us-east-1"),
]
...
# The top-level dag
dag: TtdDag = TtdDag(
    dag_id="opath-avails-pipeline-{0}-hourly".format(aws_region),
    start_date=job_start_date,
    schedule_interval=schedule_interval_hourly,
    max_active_runs=5,
    retries=max_retries,
...
standard_cluster_tags = {"Team": OPATH.team.jira_team, "Process": "OpenPathAggregate-Avails-{0}".format(aws_region)}

cluster_task_name = "OpenPathAvailsPipelineHourly-{0}".format(aws_region)
...
job_class_name = "com.thetradedesk.availspipeline.spark.jobs.OpenPathHourlyTransformOperation"
spark_options = [
...
for source_type in source_types:
    eldorado_config_option_pairs_list = [('hourToTransform', '{{ logical_date.strftime(\"%Y-%m-%dT%H:00:00\") }}'),
                                         ('ttd.openpath.transformer', source_type), ('availsRawS3Region', aws_region),
                                         ('useDeltaFormat', 'true')]

    data_set_name = source_type_data_set_name[source_type]
    use_delta_config_name = "ttd.{0}.useDeltaVersion".format(data_set_name)
    eldorado_config_option_pairs_list.append((use_delta_config_name, 'false'))

    job_step = EmrJobTask(
        name=f"HourlyTransform_{source_type}",
    ...
    vertica_import_task = VerticaImportFromCloud(
        dag=dag.airflow_dag,
        subdag_name=source_type,
        gating_type_id=gating_type_ids[source_type],
...
wait_complete = DatasetCheckSensor(
    ...
    datasets=produced_datasets_list,
