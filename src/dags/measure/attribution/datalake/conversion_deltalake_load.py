...
from dags.measure.attribution.databricks_config import DatabricksConfig, create_databricks_workflow, \
    log_gate_sensor_task, create_attribution_dag
from dags.measure.attribution.datalake.log_datapipe_config import task_assembly_param, databricks_load_task_params, \
    log_data_pipe_param, fetch_cleanse_watermark_task, fetch_delta_load_watermark_task, delta_load_branching_task, \
...
def create_dag(cloud_provider: CloudProvider):
    ...
    log_task_expr = 'dbo.fn_Enum_Task_HourlyCleanseConversionTrackerCompleted()'
    fetch_log_watermark_task = fetch_cleanse_watermark_task(log_task_expr)
    delta_high_watermark_task_id, delta_high_watermark_task = fetch_delta_load_watermark_task(log_type="conversiontracker")
    ...
    logs_gate_sensor_ti = log_gate_sensor_task(LogNames.conversiontrackerverticaload, CloudProviders.aws)
