...
from dags.measure.attribution.datalake.log_datapipe_config import task_assembly_param, fetch_cleanse_watermark_task, \
    log_data_pipe_param, databricks_load_task_params, fetch_delta_load_watermark_task, delta_load_branching_task, \
    get_log_task_high_watermark_timestamp
...
def create_dag(cloud_provider):
    dataset_id = "bidfeedback-deltalake"
    ...
    log_task_expr = 'dbo.fn_Enum_Task_HourlyCleanseBidFeedbackCompleted()'
    fetch_log_watermark_task = fetch_cleanse_watermark_task(log_task_expr)
    delta_high_watermark_task_id, delta_high_watermark_task = fetch_delta_load_watermark_task(log_type="bidfeedback")
    ...
    delta_load_run_now_task = create_databricks_workflow(
        ...
        tasks=[
            SparkDatabricksTask(
                class_name="com.thetradedesk.attribution.datapipe.bidfeedback.BidFeedbackIncrementalLoader",
...
bidfeedback_delta_load_dag = create_dag(CloudProviders.aws)
