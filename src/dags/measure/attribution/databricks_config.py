...
def create_databricks_workflow(
    ...
    install_ttd_certificate: bool = False
) -> DatabricksWorkflow:
    job_name = f"measure-attr-{job_name}-{cloud_provider.__str__()}"
    boostrap_script_action = [
        ScriptBootstrapAction(
            name='install_ttd_certificate', path=f"{DatabricksConfig.BUILD_ARTIFACT_PATH}/InitScript/ttd_cert_install.sh"
        )
    ] if install_ttd_certificate else None
    return DatabricksWorkflow(
        ...
        tasks=tasks,
        bootstrap_script_actions=boostrap_script_action,
        retries=0
    )
...
class DatabricksConfig:
    BUILD_ARTIFACT_PATH = "s3://ttd-build-artefacts/conversion-attribution-spark/release/latest"

    DATABRICKS_VERSION = "16.4.x-scala2.12"
    LOG_DATAPIPE_SPARK_EXECUTABLE_PATH = f"{BUILD_ARTIFACT_PATH}/LogIngestionDataPipe.jar"
    ATTRIBUTION_DAG_SPARK_EXECUTABLE_PATH = f"{BUILD_ARTIFACT_PATH}/AttributionDAG.jar"
