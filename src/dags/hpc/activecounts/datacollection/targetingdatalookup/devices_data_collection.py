...
# Copy across the devices data collection non-restricted dataset from s3 to azure

avails_partition_formatted = "{{ task_instance.xcom_pull(dag_id='" + dag_name + "', task_ids='recency_check', key='" + CountsDataName.ACTIVE_TDID_DAID + "').strftime(\"%Y-%m-%d %H:00:00\") }}"

devices_data_collection_copy_task = DatasetTransferTask(
    name="copy_devices_data_collection_from_s3_to_azure",
    dataset=SibDatasources.devices_active_counts_data_collection,
    dst_dataset=SibDatasources.devices_active_counts_data_collection_non_restricted_azure,
    src_cloud_provider=CloudProviders.aws,
    dst_cloud_provider=CloudProviders.azure,
    partitioning_args=SibDatasources.devices_active_counts_data_collection.get_partitioning_args(ds_date=avails_partition_formatted),
...
dag >> recency_operator_step >> devices_data_collection_cluster >> devices_data_collection_copy_task >> final_dag_check
