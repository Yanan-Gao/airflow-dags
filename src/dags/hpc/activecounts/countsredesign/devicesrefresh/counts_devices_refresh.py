...
from dags.hpc.activecounts.countsredesign.devicesrefresh.active_counts_ingestion import dag_name as active_counts_ingestion_dag_name
...
devices_refresh_task = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=eldorado_config_option_pairs_list + [('graniteHosts', granite_hosts)],
    additional_args_option_pairs_list=additional_args_option_pairs_list,
...
###########################################
# Trigger Active Counts ingestion
###########################################


# To reduce load on Algolia, we are temporarily skipping every other ingestion to Prov
def is_active_counts_ingestion_trigger_enabled(task_instance: TaskInstance):
    trigger_active_counts_ingestion = task_instance.xcom_pull(
        task_ids='is_active_counts_ingestion_trigger_enabled',
        key='trigger_active_counts_ingestion',
        include_prior_dates=True,
        default='true'
    )
    next_trigger_active_counts_ingestion = 'false' if trigger_active_counts_ingestion == 'true' else 'true'
    task_instance.xcom_push(key="trigger_active_counts_ingestion", value=next_trigger_active_counts_ingestion)
    return trigger_active_counts_ingestion == 'true'


is_active_counts_ingestion_trigger_enabled_task = OpTask(
    op=ShortCircuitOperator(
        task_id='is_active_counts_ingestion_trigger_enabled',
        python_callable=is_active_counts_ingestion_trigger_enabled,
        dag=adag,
...
trigger_active_counts_ingestion_task = OpTask(
    op=TriggerDagRunOperator(
        task_id="trigger-active-counts-ingestion",
        trigger_dag_id=active_counts_ingestion_dag_name,
        conf={"cadence_in_hours": cadence_in_hours * 2}
    )
)
...
rotate_generation_ring_cluster >> is_active_counts_ingestion_trigger_enabled_task >> trigger_active_counts_ingestion_task >> final_dag_check
