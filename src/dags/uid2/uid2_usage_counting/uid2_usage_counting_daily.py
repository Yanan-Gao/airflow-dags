...
job_name = "uid2-usage-counting-daily"
job_schedule_interval = "0 5 * * *"
job_start_date = datetime(2025, 2, 10)
job_output_root = f'thetradedesk-bi/uid2usage/{"test/" if env.execution_env != "prod" else ""}'
...
v2_usage_processing_task = EmrJobTask(
    ...
    executable_path="s3://ttd-build-artefacts/eldorado/release-spark-3/main-spark-3/latest/eldorado-uid2-assembly.jar",
    configure_cluster_automatically=True,
    eldorado_config_option_pairs_list=[
        ('Uid2UsageLogS3RootPath', 'thetradedesk-useast-logs-2/uid2usagecapturedata/collected'),
        ('Uid2UsageReportS3RootPath', f"{job_output_root}/v2"),
        ('OutputPath', 'daily-report-' + capture_date.strftime("%Y%m%d")),
        ('CaptureFromTime', capture_date.strftime("%Y-%m-%d") + 'T00:00:00'),
    ],
)
...
v3_usage_processing_task = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=[
        ('Uid2UsageLogS3RootPath', 'thetradedesk-useast-logs-2/uid2usagecapturedata/collected'),
        ('Uid2UsageReportS3RootPath', f"{job_output_root}/v3"),
        ('OutputPath', 'daily-report-' + capture_date.strftime("%Y%m%d")),
        ('CaptureFromTime', capture_date.strftime("%Y-%m-%d") + 'T00:00:00'),
    ],
