        ...
        while len(all_issues) < total_to_fetch:
            ...
            if next_page_token:
                ...
                params = {
                    ...
                    "maxResults": min(max_results, total_to_fetch - len(all_issues)),
                    "fields": "*all"
                }
            else:
                url = f"{base_url}/rest/api/3/search/jql"
                params = {"jql": jql, "maxResults": min(max_results, total_to_fetch - len(all_issues)), "fields": "*all"}
            ...
            for issue in issues:
                ...
                if 'fields' not in issue:
                    print(f"Issue {issue.get('key', 'unknown')} missing 'fields' key. Skipping.")
                    continue
            ...
            # Check if this is the last page or no more data
            if data.get('isLast', False):
                print("Reached last page of results")
                break

            # If no nextPageToken, end pagination
            if not next_page_token:
                print("No nextPageToken found, ending pagination")
                break
