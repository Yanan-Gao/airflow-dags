...
pld_reports_envs = {
    "TTD_NPI_REPORTING_REPORTDATE": "{{ (dag_run.conf.get('REPORTDATE') if dag_run and dag_run.conf else None) or ds }}",
    ...
    "TTD_NPI_APPLICATION_GENERATORTASKTYPE": "NpiReportsGenerator",
    "TTD_NPI_APPLICATION_GENERATORTASKTRIGGER": generator_task_trigger,
    "JAVA_OPTS": java_opts
}

pld_reports_generator_task = OpTask(
    op=TtdKubernetesPodOperator(
        ...
        name="npi_pld_reports_generator",
        ...
        secrets=secrets,
        env_vars=pld_reports_envs
    )
)

ttd_dag >> pld_reports_generator_task
