...
secrets = [
    ...
    Secret(
        deploy_type="env",
        deploy_target="TTD_NPI_SNOWFLAKEDATALOADING_PROVIDERS_THROTLE_ACCESSKEY",
        secret="npi-datasource",
        key="%s_SNOWFLAKEDATALOADINGACCESSKEY" % profile.upper()
    ),
    Secret(
        deploy_type="env",
        deploy_target="TTD_NPI_SNOWFLAKEDATALOADING_PROVIDERS_THROTLE_SECRETKEY",
        secret="npi-datasource",
        key="%s_SNOWFLAKEDATALOADINGSECRETKEY" % profile.upper()
    ),
    Secret(
        deploy_type="env",
...
publisher_reports_envs = {
    "TTD_NPI_PUBLISHERREPORT_REPORTDATE": "{{ (dag_run.conf.get('REPORTDATE') if dag_run and dag_run.conf else None) or ds }}",
    "SPRING_PROFILES_ACTIVE": profile,
    "TTD_NPI_APPLICATION_GENERATORTASKTYPE": "NpiPublisherReportGenerator",
    "TTD_NPI_APPLICATION_GENERATORTASKTRIGGER": generator_task_trigger,
    "JAVA_OPTS": java_opts
}

publisher_reports_generator_task = OpTask(
    op=TtdKubernetesPodOperator(
        ...
        name="npi_publisher_reports_generator",
        ...
        secrets=secrets,
        env_vars=publisher_reports_envs
    )
)
