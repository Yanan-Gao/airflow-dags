...
dag_name = "budget-hourly-kpi-metrics"
...
dag = hourly_kpi_metrics_dag.airflow_dag

###############################################################################
# S3 dataset sources
###############################################################################

janus_kpi_click_sensor = OpTask(
    op=ExternalTaskSensor(
        task_id="janus_kpi_click_sensor",
        external_dag_id="janus-kpis-rollup-click-da-ptnr-adv-cmp-ag-hourly-agg",
...
kpi_task = DatabricksWorkflow(
    job_name="budget_hourly_kpi_job",
    cluster_name="budget_hourly_kpi_metrics_cluster",
    cluster_tags={
        "Team": "DIST",
    ...
    tasks=[
        S3PythonDatabricksTask(
            ...
            args=arguments,
            job_name="budget_hourly_kpi_metrics",
            whl_paths=[WHL_PATH],
        ),
    ],
...
# DAG Dependencies
###############################################################################
hourly_kpi_metrics_dag >> janus_kpi_click_sensor >> kpi_task >> final_dag_status_step
