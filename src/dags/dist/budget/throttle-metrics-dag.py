...
PYTHON_BASE_PATH = "s3://ttd-build-artefacts/budget-fiacre/release/latest"
WHL_PATH = f"{PYTHON_BASE_PATH}/budget_fiacre-latest-py3-none-any.whl"
ENTRY_PATH_THROTTLE = f"{PYTHON_BASE_PATH}/code/throttle_metrics_job.py"
ENTRY_PATH_THROTTLE_AGG = f"{PYTHON_BASE_PATH}/code/throttle_aggregation_job.py"
...
s3_read_prefix = f's3://ttd-budget-calculation-lake/env={env.dataset_write_env}/aggregate-pacing-statistics/v=2/metric='

# Arguments
arguments_throttle = [
    "--logLevel", "Info", "--env", env.dataset_write_env, "--run_date", run_date, "--s3_prefix_delta", s3_prefix_delta, "--s3_env_suffix",
...
throttle_task = S3PythonDatabricksTask(
    entrypoint_path=ENTRY_PATH_THROTTLE,
    args=arguments_throttle,
    job_name="budget_throttle_metrics",
    whl_paths=[WHL_PATH],
)
throttle_agg_task = S3PythonDatabricksTask(
    entrypoint_path=ENTRY_PATH_THROTTLE_AGG,
    args=arguments_throttle_agg,
    job_name="budget_throttle_metrics_aggregation",
    whl_paths=[WHL_PATH],
    depends_on=[throttle_task],
)
...
parquet_delta_task = DatabricksWorkflow(
    ...
    autoscaling_config=DatabricksAutoscalingConfig(min_workers=5, max_workers=20),
    use_photon=True,
    tasks=[
        throttle_task,
        throttle_agg_task,
