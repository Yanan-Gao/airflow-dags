...
from dags.datperf.datasets import production_adgroup_budget_dataset, r_value_calculation_result_dataset, \
    volume_control_calculation_result_dataset

from airflow.sensors.external_task import ExternalTaskSensor
from airflow.utils.state import TaskInstanceState

dag_name = "budget-cpm-controller-metrics"
...
# [data_interval_start, data_interval_end] represent the
run_date = "{{data_interval_start.strftime('%Y-%m-%dT%H:%M') }}"
version = '2'
s3_prefix = 's3://thetradedesk-mlplatform-us-east-1/model_monitor/mission_control'
...
vertica_budget_sensor = OpTask(
    op=DatasetCheckSensor(
        ...
        datasets=[
            production_adgroup_budget_dataset.with_check_type("day"),
            r_value_calculation_result_dataset.with_check_type("day"),
            volume_control_calculation_result_dataset.with_check_type("day"),
        ],
...
budget_cpm_metrics_sensor = OpTask(
    op=ExternalTaskSensor(
        task_id="budget_cpm_metrics_data_available",
...
cpm_controller_metrics_task = DatabricksWorkflow(
    job_name="cpm_controller_metrics_job",
    cluster_name="cpm_controller_metrics_cluster",
    cluster_tags={
        "Team": "DIST",
    ...
    tasks=[
        S3PythonDatabricksTask(
            ...
            args=[
                "--logLevel", "Info", "--env", env.dataset_write_env, "--run_date", run_date, "--s3_prefix", s3_prefix, "--s3_env_suffix",
                s3_env_suffix, "--daily_gauntlet_allowed_count_threshold",
                str(daily_gauntlet_allowed_count_threshold), "--version", version, "--is_virtual", is_virtual, "--is_hourly", is_hourly
            ],
            job_name="cpm_controller_metrics",
            whl_paths=[WHL_PATH],
        ),
    ],
