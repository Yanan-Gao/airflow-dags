...
custom_events_spec_path = os.path.join(os.path.dirname(__file__), 'custom_events.yaml')
...
WHL_PATH = "s3://ttd-build-artefacts/user-trail/release/3347254/latest/user_trail_pipelines-0.0.1-py3-none-any.whl"
ENTRY_PATH = "s3://ttd-build-artefacts/user-trail/release/3347254/latest/code/user_trail_pipelines/job.py"
...
def load_and_write_activities_to_s3(data_interval_start: datetime, **context):
    ...
    with open(custom_events_spec_path, "r") as file:
        spec = yaml.safe_load(file)

    data_version = spec['dataVersion']
    for item in spec["events"]:
        ...
        s3_path = f"env={env.dataset_write_env}/v={data_version}/type={type}/date={start_date}"
        logging.info(s3_path)
        s3.put_buffer(file_obj=buffer, bucket_name=bucket, key=s3_path, replace=True)
