...
class JanusExportHealthCheckDag(JanusProjectSpecificDag):
    STAGE = "export"
    JANUS_PROJECT_IDS = ["all_projects"]  # Use all_projects to match intermediate DAGs

    def __init__(
        self,
        export_name: str,
        health_check_sensor_type: str,
        start_date: datetime,
        job_task_name: str = "ExportHealthCheck",  # So we don't reuse some massively long name
        end_date: Optional[datetime] = None,
        export_time: str = CURRENT_HOUR,
        core_fleet_capacity: int = 8,
        emr_release_label: str = "emr-6.7.0",
        enable_slack_alert: bool = False,
        ...
        config_options = [
            ("JanusHealthCheckExporter.exportTime", export_time),
            ("JanusHealthCheckExporter.exportDataSet", export_name),
            ("openlineage.enable", "true"),
        ]
        if extra_config_options is not None:
            config_options += extra_config_options
        ...
        super().__init__(
            metric=f"{export_name.lower()}",
            task_class="com.thetradedesk.ds.libs.janusabtestingkpipipeline.transformers.JanusHealthCheckExporter",
            config_options=config_options,
            start_date=start_date,
            end_date=end_date,
            job_task_name=job_task_name,
        ...
        self.sensor_metric = health_check_sensor_type
        self.sensor_timeout = sensor_timeout

    def setup_cluster_with_step(self, janus_project_id: str) -> BaseTask:
        cluster_task = super().setup_cluster_with_step(janus_project_id)
        health_check_sensor_task = OpTask(
            op=JanusHealthCheckSensor(
                metric=self.sensor_metric,
                janus_project_id=janus_project_id,
                timeout=self.sensor_timeout,  # in seconds
            )
        )
        health_check_sensor_task >> cluster_task
        return health_check_sensor_task
...
class JanusHealthCheckSensor(ExternalTaskSensor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        external_dag_id = get_dag_id(INTERMEDIATE_STAGE, metric)
        if job_task_name is None:
            job_task_name = INTERMEDIATE_STAGE
