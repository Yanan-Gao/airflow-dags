...
class FindMostRecentSuccessfulDowDates(BaseOperator):
    ...
    def execute(self, context) -> str:
        ...
        successful_execution_dates = self.successful_execution_dates_for_tasks(execution_date)
        dow_to_recent_successful_execution_date = self.map_to_dow(successful_execution_dates)
    ...
    def successful_execution_dates_for_tasks(self, execution_date_str: str) -> List[date]:
        ...
        dag_runs: List[DagRun] = DagRun.find(dag_id=self.dag_id)
        dag_runs.reverse()
        if len(dag_runs) <= 0:
        ...
        for dag_run in dag_runs:
            ...
            if all([
                    dag_run.get_task_instance(task_id=task_to_check) is not None
                    and dag_run.get_task_instance(task_id=task_to_check).state == State.SUCCESS  # type: ignore
                    for task_to_check in self.target_tasks
            ]):
    ...
    @staticmethod
    def map_to_dow(successful_execution_dates: List[date]) -> Dict[int, str]:
