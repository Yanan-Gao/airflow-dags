...
_NAME = 'run_task_copy_tdid_and_hmh_from_s3_to_azure'
_PARTITIONING_ARGS = {
    "date_key": PartitioningArg(key="date", value=f'{{{{ task_instance.xcom_pull(key="{RAM_GENERATION_TIMESTAMP_KEY}") }}}}', weight=0)
}
_dataset = get_test_or_default_value(RamDatasources.dailyavails_tdid_hmh_agg_test, RamDatasources.dailyavails_tdid_hmh_agg)


class CopyTdidAndHMHData(DatasetTransferTask):
    ...
    def __init__(self):
        super().__init__(
            ...
            prepare_finalise_timeout=timedelta(minutes=60),
            num_partitions=3
        )
