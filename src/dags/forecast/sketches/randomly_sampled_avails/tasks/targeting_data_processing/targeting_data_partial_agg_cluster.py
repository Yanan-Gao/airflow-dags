...
_ON_DEMAND_WEIGHT = 4576
_NAME = 'TargetingDataPartialAggCluster'
_ADDITIONAL_CLUSTER_TAGS = {"Process": "DataElementsHMH"}
_MASTER_FLEET_INSTANCE_TYPE_CONFIGS = get_core_fleet_instance_type_configs(
    instance_types=[R6g.r6g_2xlarge().with_max_ondemand_price().with_fleet_weighted_capacity(1).with_ebs_size_gb(300)],
...
_EXTRA_DISK_CORE_FLEET_INSTANCE_TYPE_CONFIGS = [
    ...
    R6g.r6g_16xlarge().with_max_ondemand_price().with_fleet_weighted_capacity(64).with_ebs_size_gb(600)
]


class TargetingDataPartialAggCluster(EmrClusterTask):
