...
from dags.forecast.sketches.randomly_sampled_avails.constants import ETL_BASED_FORECASTS_JAR_PATH, \
    ...
    GLOBAL_JVM_SETTINGS_LIST, RAM_GENERATION_TIMESTAMP_KEY, HMH_P, HMH_R
from dags.forecast.sketches.randomly_sampled_avails.utils import get_test_or_default_value
from ttd.eldorado.aws.emr_job_task import EmrJobTask

_NAME = "SetupStageAvailsTable"
_CLASSNAME = "com.thetradedesk.etlforecastjobs.universalforecasting.ram.SetupStageAvailsTable"
...
_JVM_CONFIG_OPTIONS = [('ramGenerationTimestamp', f'{{{{ task_instance.xcom_pull(key="{RAM_GENERATION_TIMESTAMP_KEY}") }}}}'),
                       ('availsSampledRate', '1.0'), ('hmhP', str(HMH_P)), ('hmhR', str(HMH_R))]
_TEST_JVM_SETTINGS = [('ttd.ds.AdhocVectorValuesDataSet.prod.s3root', 's3a://ttd-identity/datapipeline/ram/test'),
                      ('ttd.ds.AdhocDailyAvailsVectorsDataSet.prod.s3root', 's3a://ttd-identity/datapipeline/ram/test')]
_TEST_SPARK_OPTIONS: List[Tuple[str, str]] = []


class SetupStageAvails(EmrJobTask):
