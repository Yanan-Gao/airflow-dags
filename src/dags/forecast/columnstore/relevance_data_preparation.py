...
def generate_relevance_avails_loading_jobs_for_clickhouse(date: str):
    ...
    non_deal_avails_completion_task = columnstore_sql_functions.mark_dataset_loaded(
        date=date, table_type=TableType.RelevanceV2IdMappedClickHouseAvails, xd_vendor_ids=[XdLevel.v2_no_xd_level], isClickHouse=True
    )
    deal_avails_completion_task = columnstore_sql_functions.mark_dataset_loaded(
        date=date, table_type=TableType.RelevanceV2DealOptimisedClickHouseAvails, xd_vendor_ids=[XdLevel.v2_no_xd_level], isClickHouse=True
    )
    tasks = [clickhouse_load_task, non_deal_avails_completion_task, deal_avails_completion_task]
...
def generate_relevance_audience_loading_jobs_for_clickhouse(date: str) -> ChainOfTasks:
    ...
    table_type = TableType.RelevanceV2IdMappedClickHouseAudience
