...
spark_options_list = [("executor-memory", "615G"), ("executor-cores", "96"), ("num-executors", "8"),
                      ...
                      ("conf", "spark.sql.shuffle.partitions=1536"), ("conf", "spark.dynamicAllocation.enabled=false"),
                      ("conf", "spark.sql.parquet.int96RebaseModeInRead=CORRECTED")]

segment_index_dag = TtdDag(
    dag_id="adpb-segment-index",
    start_date=datetime(2024, 8, 13, 0, 0, 0),
...
cluster = EmrClusterTask(
    name="SegmentIndexGenerationCluster",
...
segment_index_generation_step = EmrJobTask(
    name="SegmentIndexGeneration",
    class_name="jobs.segmentueindexing.SegmentUeIndexGeneratorJob",
    additional_args_option_pairs_list=spark_options_list,
    eldorado_config_option_pairs_list=[("datetime", "{{ data_interval_start.strftime(\"%Y-%m-%dT%H:00:00\") }}")],
    executable_path=jar_path
)

cluster.add_sequential_body_task(segment_index_generation_step)

check = OpTask(op=FinalDagStatusCheckOperator(dag=segment_index_dag.airflow_dag))

segment_index_dag >> cluster >> check

dag = segment_index_dag.airflow_dag
