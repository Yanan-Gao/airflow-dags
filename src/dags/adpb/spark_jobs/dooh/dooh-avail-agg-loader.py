...
job_upstream_latency = 1  # hour

job_name = 'dooh-avail-agg-loader'
job_start_date = datetime(2024, 8, 1, 14, 0)
...
gating_type_id = 2000076  # dbo.fn_enum_GatingType_ImportDoohAvailslog()
...
core_instance_type = EmrFleetInstanceTypes(
    instance_types=[
        ...
        C7g.c7g_8xlarge().with_ebs_size_gb(base_ebs_size * 8).with_max_ondemand_price().with_fleet_weighted_capacity(4),
    ],
    on_demand_weighted_capacity=20
)

dooh_avail_agg_loader_cluster = EmrClusterTask(
    name="dooh_avail_agg_loader_cluster",
...
avail_agg_loader_step = EmrJobTask(
    name="dooh_avail_agg_loader_step",
    executable_path=jar_path,
    class_name="jobs.dooh.forecasting.DoohAvailLogsAggregationJob",
    ...
    cluster_specs=dooh_avail_agg_loader_cluster.cluster_specs
)

dooh_avail_agg_loader_cluster.add_parallel_body_task(avail_agg_loader_step)

# Trigger Vertica import via DataMover by opening the Gate
avail_agg_vertica_import_open_gate = OpTask(
    op=PythonOperator(
...
dag >> get_job_run_datetime_task >> dooh_avail_agg_loader_cluster >> avail_agg_vertica_import_open_gate
