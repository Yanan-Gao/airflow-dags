...
# Parameters
dag_name = "adpb-adgroup-audience-metadata-generation"
job_start_date = datetime(2024, 8, 13, 4, 0, 0)
...
# DAG definition
adgroup_audience_metadata_generation = TtdDag(
    dag_id=dag_name,
...
cluster = EmrClusterTask(
    name="AdGroupAudienceMetaDataGeneration_Cluster",
    ...
    core_fleet_instance_type_configs=EmrFleetInstanceTypes(
        instance_types=[
            R5.r5_8xlarge().with_ebs_size_gb(1024).with_max_ondemand_price().with_fleet_weighted_capacity(1),
            R5.r5_16xlarge().with_ebs_size_gb(1024).with_max_ondemand_price().with_fleet_weighted_capacity(2),
            R5.r5_24xlarge().with_ebs_size_gb(1024).with_max_ondemand_price().with_fleet_weighted_capacity(3)
        ],
...
pfs_step = EmrJobTask(
    ...
    name="AdGroupAudienceMetaDataGeneratorStep",
    class_name="jobs.adgroupaudiencemetadata.AdGroupAudienceMetaDataGeneratorJob",
...
check = OpTask(op=FinalDagStatusCheckOperator(dag=adgroup_audience_metadata_generation.airflow_dag))

adgroup_audience_metadata_generation >> cluster >> check

dag = adgroup_audience_metadata_generation.airflow_dag
