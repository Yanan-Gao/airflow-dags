...
model_prediction_subdag_task_id = "Prediction"
...
apv3_model_prediction_dag = TtdDag(
    dag_id="adpb-apv3-model-prediction-full",
...
dag = apv3_model_prediction_dag.airflow_dag
...
model_generation_spark_options_list = generate_spark_option_list(30, R5.r5_12xlarge())

model_generation_fleet_cluster = EmrClusterTask(
    name="APV3_Model_Generation",
    ...
    core_fleet_instance_type_configs=EmrFleetInstanceTypes(
        instance_types=[
            R5.r5_12xlarge().with_ebs_size_gb(500).with_max_ondemand_price().with_fleet_weighted_capacity(1),
            R5.r5_24xlarge().with_ebs_size_gb(1000).with_max_ondemand_price().with_fleet_weighted_capacity(2)
        ],
...
bid_feedback_trimmer_step = EmrJobTask(
    ...
    name="APV3_Bid_Feedback_Trimmer",
...
seed_training_user_step = EmrJobTask(
    cluster_specs=model_generation_fleet_cluster.cluster_specs,
    name="APV3_Seed_Training_Users_Generation",
...
training_data_step = EmrJobTask(
    cluster_specs=model_generation_fleet_cluster.cluster_specs,
    name="APV3_Training_Data_Generation",
...
def generate_user_scoring_clusters(batchId):
    model_prediction_fleet_cluster = EmrClusterTask(
        name=f"APV3_Model_Prediction_{batchId + 1}_of_{batchSize}",
        master_fleet_instance_type_configs=EmrFleetInstanceTypes(
            instance_types=[R5.r5_4xlarge().with_ebs_size_gb(512).with_fleet_weighted_capacity(1)],
            on_demand_weighted_capacity=1,
        ),
        ...
        core_fleet_instance_type_configs=EmrFleetInstanceTypes(
            instance_types=[
                R5.r5_12xlarge().with_ebs_size_gb(600).with_max_ondemand_price().with_fleet_weighted_capacity(1),
                R5.r5_24xlarge().with_ebs_size_gb(1200).with_max_ondemand_price().with_fleet_weighted_capacity(2)
            ],
    ...
    model_prediction_spark_options_list = generate_spark_option_list(80, R5.r5_12xlarge())

    user_scoring_step = EmrJobTask(
        cluster_specs=model_prediction_fleet_cluster.cluster_specs,
        name="APV3_User_Scoring",
...
export_cluster = EmrClusterTask(
    ...
    core_fleet_instance_type_configs=EmrFleetInstanceTypes(
        instance_types=[
            R5.r5_4xlarge().with_ebs_size_gb(400).with_max_ondemand_price().with_fleet_weighted_capacity(1),
            R5.r5_8xlarge().with_ebs_size_gb(800).with_max_ondemand_price().with_fleet_weighted_capacity(2),
            R5.r5_12xlarge().with_ebs_size_gb(1200).with_max_ondemand_price().with_fleet_weighted_capacity(3),
            R5.r5_16xlarge().with_ebs_size_gb(1600).with_max_ondemand_price().with_fleet_weighted_capacity(4),
            R5.r5_24xlarge().with_ebs_size_gb(2400).with_max_ondemand_price().with_fleet_weighted_capacity(6)
        ],
        on_demand_weighted_capacity=90
    ),
    additional_application_configurations=copy.deepcopy(application_configuration),
    enable_prometheus_monitoring=True,
    emr_release_label=AwsEmrVersions.AWS_EMR_SPARK_3_3,
...
export_spark_options_list = [("executor-memory", "100G"), ("executor-cores", "16"),
                             ("conf", "spark.executor.extraJavaOptions=-server -XX:+UseParallelGC"),
                             ("conf", "dfs.client.use.datanode.hostname=true"), ("conf", "spark.driver.memory=109G"),
                             ("conf", "spark.sql.shuffle.partitions=2700"), ("conf", "spark.driver.maxResultSize=109G"),
                             ("conf", "spark.sql.parquet.int96RebaseModeInRead=CORRECTED")]
...
uid2_export_cluster = EmrClusterTask(
    ...
    core_fleet_instance_type_configs=EmrFleetInstanceTypes(
        instance_types=[
            M6g.m6g_4xlarge().with_max_ondemand_price().with_fleet_weighted_capacity(1),
            M6g.m6g_8xlarge().with_max_ondemand_price().with_fleet_weighted_capacity(2),
            M6g.m6g_12xlarge().with_max_ondemand_price().with_fleet_weighted_capacity(3),
            M6g.m6g_16xlarge().with_max_ondemand_price().with_fleet_weighted_capacity(4)
        ],
...
uid2_export_step = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=[("runTime", run_time), ("numParallelDataServerRequests", 1000), ("numRecordsPerRequest", 250)],
    executable_path=jar_path
)
...
check = OpTask(op=FinalDagStatusCheckOperator(dag=apv3_model_prediction_dag.airflow_dag))

for user_scoring_cluster in user_scoring_clusters:
    model_prediction_start >> user_scoring_cluster >> model_prediction_end

apv3_model_prediction_dag >> model_generation_fleet_cluster >> model_prediction_start >> model_prediction_end >> export_cluster >> budget_metadata_sql_import_open_gate >> check
