...
DICT_CONFIG = {
    ...
    "prod": {
        ...
        "application_data_url": "s3://ttd-datprd-us-east-1/backstage/30d/prod/inMarketSegmentsWithFrequency",
        "slack_channel": "#scrum-data-products-alarms",
    }
}

name = "in_market_segments"
group_id = "com.thetradedesk.seggen-inMarket"
version = DICT_CONFIG[env]["version"]

start_date = datetime(2025, 7, 28, 0, 0)
schedule_interval = "0 3,9,15,21 * * *"
timeout_hours = 192
slack_channel = DICT_CONFIG[env]["slack_channel"]
cluster_name_prefix = "DATPRD-In-Market-Segments"
s3_bucket = "ttd-datprd-us-east-1"
s3_folder = DICT_CONFIG[env]["s3_folder"]

semantic_embeddings_url = "s3://ttd-contextual/prod/parquet/cxt_semantic_embeddings/v=1"
unsampled_avails_url = "s3://thetradedesk-useast-avails/datasets/withPII/prod/identity-avails-agg-hourly-v2-delta"
...
in_market_with_frequency_job = EmrJobTask(
    name="run_in_market_with_frequency",
    cluster_specs=create_cluster_step.cluster_specs,
    class_name="com.thetradedesk.seggen.inMarket.Main",
    additional_args_option_pairs_list=additional_spark_args,
    command_line_arguments=[
        "inMarketSegment", "--env", env, "--segmentWithFrequencyOutput", DICT_CONFIG[env]["application_data_url"],
        "--semanticEmbeddingsBasePath", semantic_embeddings_url, "--unsampledAvailsBasePath", unsampled_avails_url, "--startDate",
        '{{ data_interval_start.strftime("%Y-%m-%d") }}', "--startHour", '{{ data_interval_start.strftime("%H") }}',
        "--purchaseIntentionScoreThreshold", "0.666", "--relevanceThreshold", "0.4"
    ],
...
create_cluster_step.add_sequential_body_task(in_market_with_frequency_job)
