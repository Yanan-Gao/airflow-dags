...
job_start_time = "{{ data_interval_start.strftime('%Y-%m-%d %H:00:00') }}"
...
registry.register_dag(
    TaskServiceDagFactory(
        task_name="ApplyAdGroupCancelOutBatchTask",
        task_config_name="ApplyAdGroupCancelOutBatchTaskConfig",
        scrum_team=DATPERF.team,
        start_date=datetime.now() - timedelta(hours=3),
        job_schedule_interval=timedelta(minutes=30),  # Scheduled for every 30 mins, but can take 30-35 mins
        resources=TaskServicePodResources.medium(),
...
registry.register_dag(
    TaskServiceDagFactory(
        task_name="AddAdGroupCancelOutBatchTask",
        task_config_name="AddAdGroupCancelOutBatchTaskConfig",
        scrum_team=DATPERF.team,
        start_date=datetime.now() - timedelta(hours=3),
        job_schedule_interval=timedelta(minutes=60),  # Scheduled for every 60 mins
        resources=TaskServicePodResources.large(),
        task_data=json.dumps({
            "JobRunTime": job_start_time,
        })
    )
)
