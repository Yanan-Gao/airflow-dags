...
dag_name = "perf-automation-viewability-ftrl-model"

DATE = '{{ data_interval_start.strftime("%Y%m%d") }}'

docker_registry = "production.docker.adsrvr.org"
docker_image_name = "ttd-base/scrum-datperf/view_prediction"
docker_image_tag = "latest"

CLUSTERSETUP = "s3://thetradedesk-mlplatform-us-east-1/libs/viewability/scripts/clustersetup.sh"
MODELRUN = "s3://thetradedesk-mlplatform-us-east-1/libs/viewability/scripts/modelrun.sh"

ENV = TtdEnvFactory.get_from_system()

if ENV.execution_env.lower() == "prodtest":
    CLUSTERSETUP = f"s3://thetradedesk-mlplatform-us-east-1/libs/viewability/scripts/mergerequests/{docker_image_tag}/clustersetup.sh"
    MODELRUN = f"s3://thetradedesk-mlplatform-us-east-1/libs/viewability/scripts/mergerequests/{docker_image_tag}/modelrun.sh"

MODELVERSION = '2'  # string for shell script
EPOCHS = '15'
SAMPLE_SIZE = .05
LOOKBACK_DAYS = 7
####################################################################################################################
...
viewability_dag = TtdDag(
    ...
    tags=['DATPERF', "viewability"],
    enable_slack_alert=False,
)

dag = viewability_dag.airflow_dag
...
viewability_dag >> video_event_dataset_sensor >> bidrequest_dataset_sensor >> etl_cluster
