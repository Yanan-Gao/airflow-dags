...
dag_name = "perf-automation-arete-voyager-arete"
...
MODEL_NAME = 'arete'
ARETE_PREDICT_FLAG = '--if_arete'
COLD_START_PREDICT_FLAG = '--no-if_cold_start'
...
# The top-level dag
# Arete model runs at UTC 4:00, AreteVoyager has dependency on it.
arete_voyager_arete_dag = TtdDag(
    dag_id=dag_name,
...
dag = arete_voyager_arete_dag.airflow_dag
...
exog_dataset = DateGeneratedDataset(
    bucket="thetradedesk-mlplatform-us-east-1",
    path_prefix="data",
    data_name="value-pacing-forecasting/arete_exog_data",
    env_aware=True,
    version=None
)
...
# Arete - exog data
arete_exog_sensor = OpTask(
    op=DatasetCheckSensor(
        task_id='exog_data_available',
        # looks for success file in hour 23 of yesterday's folder
...
arete_voyager_task = PySparkEmrTask(
    name="AreteVoyager",
...
cluster.add_parallel_body_task(arete_voyager_task)

# Final status check to ensure that all tasks have completed successfully
final_dag_status_step = FinalDagStatusCheckOperator(dag=dag)

# DAG dependencies
arete_voyager_arete_dag >> vc_budget_sensor >> cluster
arete_voyager_arete_dag >> provisioning_adgroup_sensor >> cluster
arete_voyager_arete_dag >> arete_exog_sensor >> cluster
