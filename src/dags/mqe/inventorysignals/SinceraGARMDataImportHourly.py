...
job_start_date = datetime(2025, 3, 4)
job_schedule_interval = "45 * * * *"
max_retries: int = 3
retry_delay: timedelta = timedelta(minutes=30)

job_environment = TtdEnvFactory.get_from_system()

dag_id = "mqe-sincera-garm-data-import-hourly"
if job_environment != TtdEnvFactory.prod:
    dag_id = "mqe-sincera-garm-data-import-hourly-test"
...
master_fleet_instance_type_configs = EmrFleetInstanceTypes(
    instance_types=[
        M5.m5_xlarge().with_fleet_weighted_capacity(1).with_ebs_size_gb(128)  # Set if cluster requires additional storage
        # .with_ebs_iops(3000)
...
core_fleet_instance_type_configs = EmrFleetInstanceTypes(
    instance_types=[
        M5.m5_4xlarge().with_fleet_weighted_capacity(2).with_ebs_size_gb(128)  # Set if cluster requires additional storage
        # .with_ebs_iops(3000)
        # .with_ebs_throughput(125)
...
job_class_name = "jobs.inventorysignals.sinceraimport.SinceraGARMDataImportHourly"
...
vertica_import_datetime = '{{ data_interval_start.strftime(\"%Y-%m-%d %H:00:00\") }}'

SINCERA_GARM_CATEGORIES_METRICS_GATING_TYPE_ID = 2000558  # ImportSinceraGARMCategoriesData

garm_categories_metrics_open_gate_task = VerticaImportFromCloud(
    dag=dag.airflow_dag,
    subdag_name="GARMCategoriesHourly",
    gating_type_id=SINCERA_GARM_CATEGORIES_METRICS_GATING_TYPE_ID,
    log_type_id=LogTypeFrequency.HOURLY.value,
    log_start_time=vertica_import_datetime,
    vertica_import_enabled=True,
    job_environment=job_environment,
)

job_task >> garm_categories_metrics_open_gate_task
