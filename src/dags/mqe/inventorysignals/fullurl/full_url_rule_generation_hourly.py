...
job_monitoring_class_name = "jobs.inventorysignals.fullurl.FullURLAlgoMonitoring"
...
job_monitoring_task = EmrJobTask(
    name="MQEFullUrlRuleGeneration-MonitoringTask",
    class_name=job_monitoring_class_name,
    executable_path=jar_path,
    configure_cluster_automatically=True,
    additional_args_option_pairs_list=spark_options_list,
    eldorado_config_option_pairs_list=eldorado_config_option_pairs_list,
...
# Master node config
monitoring_master_fleet_instance_type_configs = EmrFleetInstanceTypes(
    instance_types=[C7g.c7g_2xlarge().with_fleet_weighted_capacity(1)], on_demand_weighted_capacity=1
)

# Core nodes config
monitoring_core_fleet_capacity = 20
monitoring_core_fleet_instance_type_configs = EmrFleetInstanceTypes(
    instance_types=[C7g.c7g_2xlarge().with_fleet_weighted_capacity(1)], on_demand_weighted_capacity=monitoring_core_fleet_capacity
)

# Cluster task
cluster_monitoring_task = EmrClusterTask(
    name="MQEFullUrlRuleGenerationMonitoring-Cluster",
    master_fleet_instance_type_configs=monitoring_master_fleet_instance_type_configs,
    cluster_tags={"Team": slack_groups.mqe.jira_team},
    core_fleet_instance_type_configs=monitoring_core_fleet_instance_type_configs,
    enable_prometheus_monitoring=True,
    cluster_auto_terminates=True,
    environment=job_environment,
    emr_release_label=emr_release_label
...
cluster_monitoring_task.add_parallel_body_task(job_monitoring_task)

job_task >> etl_monitoring_done
etl_monitoring_done >> cluster_monitoring_task
