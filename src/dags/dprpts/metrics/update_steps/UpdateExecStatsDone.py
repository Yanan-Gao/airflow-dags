Pre = [
    'truncate table ttd_dpsr.metrics_ExecutionStatsStage{TableSuffix}', "truncate table ttd_dpsr.metrics_NewlyResolved{TableSuffix}", """
...
select
    ...
    met.ScheduleId,
    met.DateStart,
    esac_uni.FirstDelivered,
    esac_uni.FirstStarted,
    datediff(s, met.DateStart, esac_uni.FirstDelivered),
    case when met.DateStart < esac_uni.FirstResolved then datediff(s, met.DateStart, esac_uni.FirstResolved) else 0 end,
    datediff(s, esac_uni.FirstResolved, esac_uni.FirstStarted),
    datediff(s, esac_uni.FirstStarted, esac_uni.FirstDelivered),
    met.DateRangeHours,
    2,
    esac_uni.FirstReportProviderID,
    ...
    case
        ...
        when msladc.SLASeconds is not null then datediff(s, met.DateStart, esac_uni.FirstDelivered) - msladc.SLASeconds
        else case
            when msa.IsSingleRun = 1 then datediff(s, met.DateStart, esac_uni.FirstDelivered) - 6*60*60
            else datediff(s, met.DateStart, esac_uni.FirstDelivered) - 6*60*60
        end
...
from -- ExecutionStatesAndCounts
    ...
    select
        ...
        max(esac.FirstResolved) as FirstResolved,
        max(esac.FirstStarted) as FirstStarted,
        max(esac.FirstDelivered) as FirstDelivered,
        max(esac.FirstReportProviderID) as FirstReportProviderID,
        ...
        (
            ...
            first_value(case when esh.NewStateId = 2 then esh.TransitionDate end ignore nulls) over (execution rows between unbounded preceding and current row) as FirstDelivered,
            first_value(case when esh.NewStateId = 2 then esh.ReportProviderSourceId end ignore nulls) over (execution rows between unbounded preceding and current row) as FirstReportProviderID,
...
where esac_uni.FirstDelivered is not null and esac_uni.FirstStarted is not null and esac_uni.FirstResolved is not null
"""
