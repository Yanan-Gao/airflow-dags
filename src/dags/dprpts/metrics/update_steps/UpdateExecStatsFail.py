...
select
    ...
    case when met.DateStart < esac_uni.LastFailed then datediff(s, met.DateStart, esac_uni.LastFailed) else 0 end,
    ...
    case when esac_uni.FirstStarted is not null and met.DateStart < esac_uni.FirstStarted then datediff(s, esac_uni.FirstStarted, esac_uni.LastFailed) else 0 end,
    met.DateRangeHours,
    3,
    ...
    case
        ...
        when msladc.SLASeconds is not null then datediff(s, met.DateStart, esac_uni.LastFailed) - msladc.SLASeconds
        else case
            when msa.IsSingleRun = 1 then datediff(s, met.DateStart, esac_uni.LastFailed) - 6*60*60
            else datediff(s, met.DateStart, esac_uni.LastFailed) - 6*60*60
        end
...
from -- ExecutionStatesAndCounts
    ...
    select
        ...
        max(esac.LastFailed) as LastFailed,
    ...
    from (
        select
            ...
            last_value(case when esh.NewStateId = 3 then esh.TransitionDate end ignore nulls) over (execution rows between unbounded preceding and current row) as LastFailed,
            last_value(case when esh.NewStateId = 3 then esh.ReportProviderSourceId end ignore nulls) over (execution rows between current row and unbounded following) as LastReportProviderID,
        ...
        and not exists (select mest.ScheduleExecutionId from ttd_dpsr.metrics_ExecutionStats{TableSuffix} mest where mest.ScheduleExecutionId = esh.ScheduleExecutionId)
