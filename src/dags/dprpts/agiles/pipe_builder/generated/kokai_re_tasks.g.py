...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiReRtiKokaiReReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-re-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiReRtiKokaiReReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-re-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiReRtiKokaiReReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-re-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-re').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
