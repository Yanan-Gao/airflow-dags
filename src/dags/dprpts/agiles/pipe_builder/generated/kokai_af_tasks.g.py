...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiAfRtiKokaiAfReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-af-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiAfRtiKokaiAfReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-af-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiAfRtiKokaiAfReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-af-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-af').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
