...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiXRtiKokaiXReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-x-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiXRtiKokaiXReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-x-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiXRtiKokaiXReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-x-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-x').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
