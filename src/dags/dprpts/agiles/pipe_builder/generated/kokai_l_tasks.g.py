...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiLRtiKokaiLReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-l-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiLRtiKokaiLReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-l-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiLRtiKokaiLReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-l-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-l').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
