...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiOverRtiKokaiOverviewReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-overview-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiOverRtiKokaiOverviewReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-overview-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiOverRtiKokaiOverviewReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-overview-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-over').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
