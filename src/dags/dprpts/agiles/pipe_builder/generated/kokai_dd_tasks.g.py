...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiDdRtiKokaiDdReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-dd-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiDdRtiKokaiDdReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-dd-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiDdRtiKokaiDdReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-dd-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-dd').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
