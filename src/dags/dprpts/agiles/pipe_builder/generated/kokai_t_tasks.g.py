...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiTRtiKokaiTReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-t-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiTRtiKokaiTReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-t-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiTRtiKokaiTReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-t-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-t').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
