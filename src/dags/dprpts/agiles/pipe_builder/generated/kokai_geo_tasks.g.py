...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiGeoRtiKokaiGeoReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-geo-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiGeoRtiKokaiGeoReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-geo-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiGeoRtiKokaiGeoReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-geo-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-geo').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
