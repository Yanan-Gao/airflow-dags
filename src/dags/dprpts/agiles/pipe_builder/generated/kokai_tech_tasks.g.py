...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiTechRtiKokaiTechReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-tech-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiTechRtiKokaiTechReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-tech-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiTechRtiKokaiTechReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-tech-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-tech').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
