...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiCreRtiKokaiCreReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-cre-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiCreRtiKokaiCreReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-cre-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiCreRtiKokaiCreReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-cre-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-cre').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
