...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiSiteRtiKokaiSiteReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-site-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiSiteRtiKokaiSiteReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-site-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiSiteRtiKokaiSiteReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-site-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-site').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
