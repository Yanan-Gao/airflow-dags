...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiWRtiKokaiWReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-w-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiWRtiKokaiWReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-w-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiWRtiKokaiWReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-w-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-w').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
