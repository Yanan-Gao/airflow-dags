...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiFpRtiKokaiFpReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-fp-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiFpRtiKokaiFpReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-fp-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiFpRtiKokaiFpReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-fp-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-fp').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
