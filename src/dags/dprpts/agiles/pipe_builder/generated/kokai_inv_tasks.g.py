...
def build_consistency_dag(dag: TtdDag, task_builder: ConsistencyTaskBuilder) -> None:
    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiInvRtiKokaiInvReportAggregationCheckConfig",
        task_name_suffix="rti-kokai-inv-true-up",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiInvRtiKokaiInvReportLateDataAggregationCheckConfig",
        task_name_suffix="rti-kokai-inv-late-data",
    )

    dag >> task_builder.create_consistency_check(
        task_config_name="KokaiInvRtiKokaiInvReportIntegralAggregationCheckConfig",
        task_name_suffix="rti-kokai-inv-integral",
    )


consistency_dag = ConsistencyDagBuilder(flow_name='kokai-inv').build(lambda dag, task_builder: build_consistency_dag(dag, task_builder))
