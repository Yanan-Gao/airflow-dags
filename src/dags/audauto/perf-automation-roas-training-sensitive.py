...
DEFAULT_ROLLOUT_FEATURE_FLAG_NAME = "revenue-restricted-model-rollout"
...
virtual_cores = 72
feature_json_file = "feature_roas_sensitive.json"
...
roas_training_dag = TtdDag(
    dag_id="perf-automation-roas-training-restricted",
...
training_docker_command = DockerCommandBuilder(
    ...
    additional_execution_parameters=[
        ...
        "instance.push_summary_stats=False", "training.train_sensitive_model=True"
...
def generate_scoring_subtask_for_date(scoring_date: str, index: int):
    ...
    feature_json_argument = ["{% raw %}--feature_json_file=" + feature_json_file + "{% endraw %}"]
    scoring_docker_command = DockerCommandBuilder(
        ...
        additional_execution_parameters=[
            ...
            f"--scoring_batch_size={scoring_batch_size}", f"--virtual_cores={virtual_cores}", "--scoring_sensitive_model=True",
            "--neofy_roas_model=0"
        ] + feature_json_argument + env_specific_argument_kongming_python
    )
...
neofy_calibration_docker_command = DockerCommandBuilder(
    ...
    additional_execution_parameters=[
        f"train_set={train_set}", f"instance={instance_type}", f"instance.env={environment}", f"instance.model_creation_date={model_date}",
        "instance.push_summary_stats=False", "train_set.translate_model_to_tensorflow=True", "training.train_sensitive_model=True"
    ] + env_specific_argument_lantern,
)
...
register_docker_command = DockerCommandBuilder(
    ...
    additional_execution_parameters=[
        ...
        "--neofy_roas_model=1", "--scoring_sensitive_model=True"
    ] + env_specific_argument_kongming_python
)
...
if environment == TtdEnvFactory.prod and promote_to_rollout:
    ...
    model_rollout_command_line_arguments = [
        f'--rollout_strategy={ROLLOUT_STRATEGY}', f'--model_name={MODEL_NAME}', f'--success_threshold={SUCCESS_THRESHOLD}',
