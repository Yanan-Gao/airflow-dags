...
spark_options_list = [("executor-memory", "204G"), ("executor-cores", "32"),
                      ...
                      ("conf", "spark.driver.cores=15"), ("conf", "spark.driver.maxResultSize=50G"),
...
rsmv2_etl_inc_cluster_tasks = dict([(
    f"{cfg['name']}",
    EmrClusterTask(
        name=f"RSMV2_ETL_Inc_Cluster_{cfg['name']}",
        master_fleet_instance_type_configs=EmrFleetInstanceTypes(
            instance_types=[R5.r5_4xlarge().with_ebs_size_gb(128).with_fleet_weighted_capacity(1)],
            on_demand_weighted_capacity=1,
        ),
        ...
        core_fleet_instance_type_configs=EmrFleetInstanceTypes(
            ...
            on_demand_weighted_capacity=cfg["capacity"]
        ),
        emr_release_label=emr_release_label,
...
def create_rsm_job_task(name, eldorado_config_specific_list, specific_spark_option_list) -> EmrJobTask:
    # common config
    ...
    return EmrJobTask(
        ...
        additional_args_option_pairs_list=(
            copy.deepcopy(spark_options_list) + specific_spark_option_list +
...
rsm_full_tasks_config = [
    {
        ...
        "sparkOptionList": [("conf", "spark.sql.shuffle.partitions=16384"), ("conf", "spark.default.parallelism=8192")]
    },
    {
        "name":
...
rsm_inc_tasks_config = [
    {
        ...
        "sparkOptionList": [("conf", "spark.sql.shuffle.partitions=12288"), ("conf", "spark.default.parallelism=6144")]
    },
    {
        "name":
        ...
        "config": [('optInSeedFilterExpr', "'Source=6 and Tag=4'"), ("subFolder", "TTDOwnDataNonsensitiveSmall"),
                   ...
                   ("densityFeatureReadPathWithoutSlash", f"'{featureReadPathPrefix}/Source=TTDOwnData/FeatureKey=SiteZip/v=1'")],
        "sparkOptionList": [("conf", "spark.sql.shuffle.partitions=20480"), ("conf", "spark.default.parallelism=10240")]
    },
    {
        "name":
    ...
    {
        "name":
        "RelevanceModelInputGeneratorSeedSensitiveNew",
        "clusterSplit":
        "Sensitive",
        "config":
        [('optInSeedFilterExpr', "'Source=3 and Tag=2 and IsSensitive'"), ("subFolder", "SeedSensitiveNew"),
         ("upLimitPosCntPerSeed", "40000"), ("sensitiveFeatureColumn", "'Site,Zip'"),
         ("densityFeatureReadPathWithoutSlash", f"'{featureReadPathPrefix}/Source=Seed/FeatureKey=AliasedSupplyPublisherIdCity/v=1'")],
        "sparkOptionList": [("conf", "spark.sql.shuffle.partitions=12288"), ("conf", "spark.default.parallelism=6144")]
    },
    {
        "name":
        "RelevanceModelInputGeneratorSeedNonsensitiveNew",
        "clusterSplit":
        "NonSensitive",
        "config": [('optInSeedFilterExpr', "'Source=3 and Tag=2 and not IsSensitive'"), ("subFolder", "SeedNonsensitiveNew"),
...
for cfg in rsm_full_tasks_config:
    rsmv2_etl_full_cluster_task.add_parallel_body_task(create_rsm_job_task(cfg["name"], cfg["config"], cfg["sparkOptionList"]))

for cfg in rsm_inc_tasks_config:
    rsmv2_etl_inc_cluster_tasks[str(cfg["clusterSplit"]
                                    )].add_parallel_body_task(create_rsm_job_task(cfg["name"], cfg["config"], cfg["sparkOptionList"]))
...
rsmv2_etl_concurrent_inc_cluster_tasks = [
    set_step_concurrency(rsmv2_etl_inc_cluster_task, 3) for rsmv2_etl_inc_cluster_task in rsmv2_etl_inc_cluster_tasks.values()
]
...
for rsmv2_etl_concurrent_inc_cluster_task in rsmv2_etl_concurrent_inc_cluster_tasks:
