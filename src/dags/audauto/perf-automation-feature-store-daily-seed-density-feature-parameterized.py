...
new_seed_meta_site_zip_dataset: DateGeneratedDataset = DateGeneratedDataset(
    bucket="thetradedesk-mlplatform-us-east-1",
    path_prefix=f'features/feature_store/{env}/profiles/source=bidsimpression/index=SeedId/metadata',
    data_name="newSeedSiteZip",
    date_format="date=%Y%m%d",
    version=1,
    env_aware=False,
)

new_seed_meta_aliased_supply_vendor_id_city_dataset: DateGeneratedDataset = DateGeneratedDataset(
    bucket="thetradedesk-mlplatform-us-east-1",
    path_prefix=f'features/feature_store/{env}/profiles/source=bidsimpression/index=SeedId/metadata',
    data_name="newSeedAliasedSupplyPublisherIdCity",
    date_format="date=%Y%m%d",
    version=1,
    env_aware=False,
)
...
new_seed_sensor = DatasetCheckSensor(
    ...
    timeout=60 * 60 * 8,
    ds_date='{{data_interval_start.to_datetime_string()}}',
    datasets=[new_seed_meta_site_zip_dataset, new_seed_meta_aliased_supply_vendor_id_city_dataset],
)
