...
offline_feature_store_dag = TtdDag(
    dag_id="perf-automation-feature-store-hourly-seed-feature-pair-count",
...
master_fleet_instance_configs = EmrFleetInstanceTypes(
    instance_types=[R7gd.r7gd_16xlarge().with_fleet_weighted_capacity(1)],
    on_demand_weighted_capacity=1,
)

core_fleet_instance_configs = EmrFleetInstanceTypes(
    instance_types=[
        # R5.r5_16xlarge().with_ebs_size_gb(512).with_ebs_throughput(750).with_max_ondemand_price().with_fleet_weighted_capacity(1),
        R7gd.r7gd_16xlarge().with_ebs_size_gb(512).with_ebs_throughput(750).with_max_ondemand_price().with_fleet_weighted_capacity(1),
    ],
...
agg_job_dict = {
    "HourlySeedFeaturePairCount": ["SeedId"],
}
...
for job_name, profile_list in agg_job_dict.items():
    ...
    for pl in profile_list:
        agg_task = EmrJobTask(
            ...
            eldorado_config_option_pairs_list=java_settings_list + [("date", DATE_MACRO), ("hour", hour_str), ("numPartitions", 8192)],
