...
spark_options = [("executor-memory", "76g"), ("executor-cores", "10"), ("conf", "spark.driver.memory=76g"),
                 ("conf", "spark.driver.cores=10"), ("conf", "spark.driver.maxResultSize=76g"),
                 ...
                 ("conf", "spark.sql.legacy.parquet.int96RebaseModeInRead=CORRECTED"), ("conf", "spark.default.parallelism=%s" % 8000),
                 ("conf", "spark.sql.shuffle.partitions=%s" % 20000)]
...
offline_feature_store_dag = TtdDag(
    dag_id="perf-automation-feature-store-hourly-initial-aggregation",
...
contextual_dataset: HourGeneratedDataset = HourGeneratedDataset(
    bucket="thetradedesk-mlplatform-us-east-1",
    path_prefix="prod/features/data",
    data_name="contextualwithbid",
...
impressions_data = HourGeneratedDataset(
    bucket="thetradedesk-mlplatform-us-east-1",
...
data_sensor = DatasetCheckSensor(
    dag=dag,
    task_id="upsream_data_available",
    poke_interval=60 * 10,
    timeout=60 * 60 * 6,
    ds_date='{{data_interval_start.to_datetime_string()}}',
    datasets=[contextual_dataset],
)
...
core_fleet_instance_config = EmrFleetInstanceTypes(
    instance_types=[
        R5.r5_16xlarge().with_ebs_size_gb(1024).with_max_ondemand_price().with_fleet_weighted_capacity(1),
    ],
    on_demand_weighted_capacity=num_workers,
)
# JOB DICT - <Data_Source>
agg_job_dict = ["BidContextual", "Impressions", "BidsImpressions"]

agg_clusters = []

for data_source in agg_job_dict:
    ...
    feature_store_agg_job_cluster = EmrClusterTask(
        name=f"Hourly_InitialAgg_{data_source}",
        cluster_tags={
    ...
    agg_task = EmrJobTask(
        ...
        eldorado_config_option_pairs_list=java_settings_list + [("date", DATE_MACRO), ("hour", hour_str), ("aggDataSource", data_source)],
