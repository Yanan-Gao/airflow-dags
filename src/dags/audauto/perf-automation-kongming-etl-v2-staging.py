...
job_setting_list = ([("date", "{{ds}}"), ("task", "cpa"), ("conversionLookback", 1), ("AdGroupPolicyGenerator.GlobalDataLookBack", 3),
                     ("AdGroupPolicyGenerator.TryMaintainConfigValue", "false"), ("AdGroupPolicyGenerator.PositivesCountWarmUpDays", 0),
                     ("ttd.env", str(environment)), ("clickLookback", 0),
...
calibration_oos_lookback = (1, 1)  # imp, conv lookback to generate labeled calibration data

# OOS Configs for Staging ETL (impression lookback, conversion lookback)
# Only run OOS Timely to save resources, 100% code coverage since OOS GT job is the same
oos_lookback_timely = (1, 1)
#################################################################
# End of Setting Area
#################################################################
...
kongming_etl_dag = TtdDag(
    dag_id="perf-automation-kongming-etl-v2-staging",
...
kongming_etl_cluster = utils.create_emr_cluster("KongmingEtlCluster", num_clusters, master_ebs_gb=300, core_ebs_per_x=core_node_ebs_per_x)
...
daily_score_set >> oos_attribute_tly_task
