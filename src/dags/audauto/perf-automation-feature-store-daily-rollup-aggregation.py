...
def create_dataset(source: str, index: str, job: str, is_hourly: bool = True) -> Any:
    """Helper function to create datasets with consistent configuration."""
    base_config = {
        "bucket": "thetradedesk-mlplatform-us-east-1",
        "path_prefix": "features/feature_store/prod/profiles",
        "data_name": f"source={source}/index={index}/job={job}",
    ...
    if is_hourly:
        base_config["hour_format"] = "hour={hour}"
        return HourGeneratedDataset(**base_config)
    else:
        return DateGeneratedDataset(**base_config)
...
dataset_configs: List[DatasetConfig] = [
    # Contextual datasets
    {
        "source": "contextualwithbid",
        "index": "AdvertiserId",
        "job": "HourlyInitialAggJob",
        "is_hourly": True
    },
    {
        "source": "contextualwithbid",
    ...
    },
    {
        "source": "contextualwithbid",
        "index": "TDID",
        "job": "DailyInitialAggJob",
        "is_hourly": False
    },
    # Impressions datasets
    {
        "source": "impressions",
        "index": "AdvertiserId",
        "job": "HourlyInitialAggJob",
    ...
    # BidsImpressions initial agg datasets
    {
...
# Generate all datasets from configuration
datasets = [
    create_dataset(source=str(config["source"]), index=str(config["index"]), job=str(config["job"]), is_hourly=bool(config["is_hourly"]))
...
# JOB DICT - <DataSource> : <LIST_OF_AGG_LEVELS>
agg_datasource_dict = {
    "BidContextual": ["CampaignId", "AdvertiserId", "TDID"],
    "Impressions": ["CampaignId", "AdvertiserId", "TDID"],
    "BidsImpressions": ["CampaignId", "AdvertiserId", "TDID"]
}
