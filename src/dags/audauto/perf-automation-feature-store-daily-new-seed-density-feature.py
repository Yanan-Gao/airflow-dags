...
new_seed_metadata_dataset: DateGeneratedDataset = DateGeneratedDataset(
    bucket="thetradedesk-mlplatform-us-east-1",
    path_prefix=f'features/feature_store/{env}/profiles/source=bidsimpression/index=SeedId/metadata',
    data_name="newSeed",
    date_format="date=%Y%m%d",
    version=1,
    env_aware=False,
    success_file="_SEED_DETAIL_PATH"
)
...
new_seed_metadata_sensor = DatasetCheckSensor(
    dag=dag,
    task_id="NewSeedMetadataAvailable",
    poke_interval=60,
    timeout=60 * 60 * 8,
    ds_date='{{data_interval_start.to_datetime_string()}}',
    datasets=[new_seed_metadata_dataset],
)
...
new_seed_sensitive_task = EmrJobTask(
    ...
    eldorado_config_option_pairs_list=java_settings_list + [("date", DATE_MACRO), ('seedGroupFeatureKeys', "AliasedSupplyPublisherId,City"),
                                                            ("IsSensitive", "true"), ("readSeedDetailMode", "true")],
    executable_path=FEATURE_STORE_JAR,
    timeout_timedelta=timedelta(hours=12)
)
feature_store_new_seed_sensitive_job_cluster.add_parallel_body_task(new_seed_sensitive_task)

offline_feature_store_dag >> feature_store_new_seed_non_sensitive_job_cluster
geronimo_sensor >> policy_table_sensor >> feature_store_new_seed_non_sensitive_job_cluster.first_airflow_op()
feature_store_new_seed_non_sensitive_job_cluster.last_airflow_op()

offline_feature_store_dag >> feature_store_new_seed_sensitive_job_cluster
geronimo_sensor >> policy_table_sensor >> new_seed_metadata_sensor >> feature_store_new_seed_sensitive_job_cluster.first_airflow_op()
feature_store_new_seed_sensitive_job_cluster.last_airflow_op()
