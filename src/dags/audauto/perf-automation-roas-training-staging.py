...
env_specific_argument_lantern: List[str] = [
    f"instance.experiment={experiment_name}",
    f"train_set.dataset.structured.experiment={experiment_name}",
    f"train_set.trainset_pretrain.structured.experiment={experiment_name}",
    f"training.mapping_experiment={experiment_name}",
    f"train_set.mlflow_registry_test_name={experiment_name}",
    "training.AUC_threshold=0.01"  # lower the bar for training to be success
]

env_specific_argument_kongming_python: List[str] = [f"experiment={experiment_name}"]
...
DEFAULT_CUSTOM_INTERVALS_IN_MINUTES = "20.0,20.0,20.0"
...
calibration_weighted_capacity = 50
...
roas_training_dag = TtdDag(
    dag_id="perf-automation-roas-training-staging",
...
roas_etl_dag_sensor = ExternalTaskSensor(
    ...
    external_dag_id="perf-automation-roas-etl-staging",
...
for scoring_cluster in scoring_sub_clusters:
    model_training_cluster >> scoring_cluster >> calibration_cluster >> neofy_calibration_cluster >> register_cluster
