...
stats_date_hour_start = get_hour_by_offset(hours_buffer + hours_to_run)
stats_date_hour_end = get_hour_by_offset(hours_buffer)

cluster.add_sequential_body_task(
    IdentityClusters.task(
        class_name="com.thetradedesk.etl.misc.AvailsEtlStatsUploader",
        executable_path=Executables.etl_repo_executable,
        runDate_arg="DATE",  # Not used
        eldorado_configs=[
            ('LOCAL', 'false'),
            ('ENVIRONMENT', str(ttd_env)),
            ('DATE_TIME_FROM', stats_date_hour_start),
            ('DATE_TIME_TO', stats_date_hour_end),
            ('STATS_PATH', stats_path),
            ('PUSH_GATEWAY', 'prom-push-gateway.adsrvr.org:80'),
        ],
        timeout_hours=1
    )
)
