...
def iav2_generator_cluster(
    ...
    # TODO setting this to emr6 due to recent OOM errors seen after spark 3.5 upgrade
    # https://thetradedesk.slack.com/archives/CE17E9BC0/p1753265102671339
    graph_generator_cluster = IdentityClusters.get_cluster(
        cluster_name, dag, num_cores, ComputeType.STORAGE, cpu_bounds=(64, 2048), emr_release_label=Executables.emr_version_6
    )
...
class IdentityAllianceETL:
    ...
    def _get_etl_cluster(
        ...
        etl_cluster = IdentityClusters.get_cluster(
            f"{graph_name}-etl_cluster", dag, num_cores, ComputeType.STORAGE, use_delta=False, emr_release_label=Executables.emr_version_6
        )
    ...
    def _get_vertica_format_cluster(
        ...
        score_norm_vertica_format_cluster = IdentityClusters.get_cluster(
            f"{graph_name}-vertica-format_cluster",
            dag,
            num_cores,
            ComputeType.STORAGE,
            use_delta=False,
    ...
    def graph_extensions_cluster(name: str):
        return IdentityClusters.get_cluster(
            name, dag, num_cores, ComputeType.STORAGE, instance_configuration_spark_log4j=None, emr_release_label=Executables.emr_version_6
        )
