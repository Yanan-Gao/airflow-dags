...
def create_dag(
    ...
    if cluster_factory == IdentityClusters:
        cluster_kwargs = {"emr_release_label": Executables.emr_version_6}
    else:
        cluster_kwargs = {}

    cluster = cluster_factory.get_cluster(
        "device_atlas_s3_updater_cluster", dag, num_cores, ComputeType.STORAGE, use_delta=False, **cluster_kwargs
    )
