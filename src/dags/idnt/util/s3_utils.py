...
def get_latest_etl_date_from_path(
    s3_hook: AwsCloudStorage, output_path: str, date_from: datetime = datetime.today(), lookback_days: int = 90
) -> datetime:
    ...
    dates = [d for d in [extract_date_from_path(path) for path in prefixes_found] if d is not None]
