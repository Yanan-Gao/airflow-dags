...
check_ttdgraph_v1 = DagHelpers.check_datasets([IdentityDatasets.ttdgraph_v1])
...
def get_etl_load_cluster():
    ...
    cluster = IdentityClusters.get_cluster("etl_load_ttdgraph", dag, etl_cluster_cores, ComputeType.STORAGE)
...
def get_metrics_steps() -> List[OpTask]:
    class_names = [
        f"{Executables.identity_repo_class}.pipelines.OpenGraphMonitoring", f"{Executables.identity_repo_class}.pipelines.AdbrainMonitoring"
    ]
...
def get_bidrelevance_cluster(is_prod: bool) -> EmrClusterTask:
    ...
    cluster.add_sequential_body_task(
        IdentityClusters.task(
            ...
            eldorado_configs=[("graphs.metrics.BidRelevance.graphLookbackWindow", 9),
                              ("graphs.metrics.BidRelevance.uploadToPrometheus", str(is_prod).lower())],
            timeout_hours=8,
...
multi_week_cluster >> edge_init_cluster >> regional_clustering >> check_ttdgraph_v1 >> graph_publish_cluster
...
graph_publish_cluster >> etl_cluster >> get_bidrelevance_cluster(Tags.environment() == "prod")
