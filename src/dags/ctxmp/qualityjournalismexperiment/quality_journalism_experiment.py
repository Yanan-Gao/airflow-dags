...
QUALITY_JOURNALISM_EXPERIMENT_JAR = "s3://ttd-build-artefacts/eldorado/release-spark-3/main-spark-3/latest/eldorado-datperf-assembly.jar"

####################################################################################################################
# DAG
####################################################################################################################

quality_journalism_experiment_dag = TtdDag(
    dag_id="quality-journalism-experiment-measurement",
    start_date=datetime(2024, 8, 21),
    schedule_interval="0 6 * * *",
    dag_tsg='https://atlassian.thetradedesk.com/confluence/x/vNoMCQ',
    retries=1,
    max_active_runs=1,
    tags=['CTXMP', 'QualityJournalism'],
    slack_tags=slack_groups.CTXMP.team.name,
    slack_channel=slack_groups.CTXMP.team.alarm_channel,
    enable_slack_alert=False
)

dag = quality_journalism_experiment_dag.airflow_dag
...
cluster = EmrClusterTask(
    name="quality-journalism-experiment-measurement-job",
...
quality_journalism_experiment_reportAgg = EmrJobTask(
    name="qualityJournalismExperimentReportAgg",
    class_name="com.thetradedesk.jobs.qualityjournalismexperiment.QualityJournalismRTBReportCumulativeAgg",
    executable_path=QUALITY_JOURNALISM_EXPERIMENT_JAR,
    additional_args_option_pairs_list=spark_options_list,
    configure_cluster_automatically=True,
    eldorado_config_option_pairs_list=[("runTime", RUNTIME_DATE), ("date", JOB_DATE)]
)

cluster.add_parallel_body_task(quality_journalism_experiment_reportAgg)

quality_journalism_experiment_measurement = EmrJobTask(
    name="qualityJournalismExperimentMeasurement",
    class_name="com.thetradedesk.jobs.qualityjournalismexperiment.QualityJournalismExperiment",
    executable_path=QUALITY_JOURNALISM_EXPERIMENT_JAR,
    additional_args_option_pairs_list=spark_options_list,
    eldorado_config_option_pairs_list=[("runTime", RUNTIME_DATE), ("date", JOB_DATE)]
)

cluster.add_parallel_body_task(quality_journalism_experiment_measurement)

# Final status check to ensure that all tasks have completed successfully
final_dag_status_step = FinalDagStatusCheckOperator(dag=dag)

# DAG dependencies
quality_journalism_experiment_dag >> platform_report_sensor >> cluster
quality_journalism_experiment_dag >> bidfeedback_sensor >> cluster
quality_journalism_experiment_reportAgg >> quality_journalism_experiment_measurement
