...
def start_snapshot_generation(snapshot, task_instance):
    ...
    try:
        ...
        _logger.info(f"Sending request to {request_url} to begin snapshot generation")
        ...
        _logger.exception(f"Failed to connect to snapshot endpoint to start snapshot generation. Response: {response}.")
        raise
...
def post_snapshot_generation_status(
    ...
    if snapshot == 'MerchantProductDetails':
        ...
        if merchant_product_details_started_at not in [None, 'None']:
            data["MerchantProductDetailsSnapshotStartedAt"] = merchant_product_details_started_at
        else:
            _logger.warning("Not required to post snapshot status as MerchantProductDetails never started.")
            return
    else:
    ...
    for _ in range(_post_snapshot_generation_status_max_tries):
        ...
        try:
            request_url = f"{_product_catalog_url}/elasticsearch/snapshot/{snapshot.lower()}/status"
            _logger.info(f"Sending request to {request_url} to end snapshot generation")
            ...
            _logger.info(
                f"Post snapshot generation status for {data} returned status code {response.status_code} and content {response.text}"
            )
            ...
            _logger.warning(f"Post snapshot generation status for {data} resulted in an exception.", exc_info=True)

        _logger.info(f"Waiting for {_post_snapshot_generation_status_retry_delay_seconds} seconds before next try")
