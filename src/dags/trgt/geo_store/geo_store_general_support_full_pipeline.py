...
# add a 5-minute offset for airflow prod pipeline to avoid having the same output prefix as the delta pipeline
# and run less frequently in airflow prod test
schedule_interval = "5 */3 * * *" if TtdEnvFactory.get_from_system() == TtdEnvFactory.prod else "0 */8 * * *"
