...
#             f"{self.env}/"
#             f"{exp_dir}{self.group_name}/{self.job_name}/identity_config.yml"
#         )
#
...
#         when ``identity_config.yml`` has been uploaded previously.  It returns
#         ``True`` when ``result.yml`` is found, otherwise ``False``.
#         """
#
#         config_key = runtime_base + "identity_config.yml"
...
#         config_runtime_key = runtime_base + "identity_config.yml"
#
#         # provide runtime path to the underlying EMR task
#         self._emr_task.eldorado_config_option_pairs_list.append(("confetti_runtime_config_base_path", runtime_base))
#
