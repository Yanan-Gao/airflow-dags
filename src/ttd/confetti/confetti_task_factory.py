...
def resolve_env(env: str, experiment: str | None) -> str:
    """Return the Confetti environment name.

    ``experiment`` must be absent when running in production and present for
    all other environments. ``env`` is normalised to lowercase. The returned
    value is either ``"prod"`` or ``"test"`` depending on ``env``.
    """

    env = (env or "").lower()
    if env in ("prod", "production"):
        if experiment not in (None, ""):
            raise ValueError("experiment_name must not be set for prod env")
        return "prod"

    if not experiment:
        raise ValueError("experiment_name is required for non-prod env")
