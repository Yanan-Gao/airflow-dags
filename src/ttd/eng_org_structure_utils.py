...
ENG_ORG_STRUCTURE_TEAMS = "https://eng-teams.gen.adsrvr.org/api/v1/teams"

cache: TTLCache = TTLCache(maxsize=10 * 1024, ttl=24 * 60 * 60, getsizeof=len)


def _get_requests_session() -> Session:
    retry = Retry(
        total=3,
        backoff_factor=0.5,
...
def get_scrum_teams() -> Dict[str, Any]:
    response = _get_requests_session().get(ENG_ORG_STRUCTURE_TEAMS, timeout=30)
...
def get_alarm_channel_id(team_name: str | None) -> str | None:
