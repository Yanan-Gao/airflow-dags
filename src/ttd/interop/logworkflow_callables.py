...
def ExternalGateBatchOpen(mssql_conn_id: str, sproc_arguments: Dict[str, Any], **kwargs):
    # skipping hook.run() so we can have more control over our sql session
    hook = MsSqlHook(mssql_conn_id=mssql_conn_id, schema='LogWorkflow')
    conn = hook.get_conn()
    conn.autocommit(True)
    ...
    # open external gate for each combination of gating type & time
    for gatingType in sproc_arguments['gatingTypes']:
        for dateTimeToOpen in sproc_arguments['dateTimesToOpen']:
            execute_sproc += f"""
            SET @gatingType = '{gatingType}';
            SET @dateTimeToOpen = CAST ( '{dateTimeToOpen}' as datetime2);
            EXEC [WorkflowEngine].[prc_OpenExternalGate] @gatingType, @grain, @dateTimeToOpen, @runData = @runData
            """
    execute_sproc += "\nCOMMIT TRAN"
    logging.info(execute_sproc)
    try:
        ...
        logging.warning("failed on prc_OpenExternalGate with query: '%s'", execute_sproc)
        raise ValueError('sproc call failed')
