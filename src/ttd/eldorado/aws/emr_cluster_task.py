...
class EmrClusterTask(TaggedClusterMixin, SetupTeardownTask):
    ...
    def get_managed_bootstrap_scripts(self, region_name: str, disable_vmem_pmem_checks: bool,
                                      enable_spark_history_server_stats: bool) -> List[ScriptBootstrapAction]:
        ...
        if region_name != "eu-central-1":
            if disable_vmem_pmem_checks:
                managed_bootstrap_scripts += [
                    ...
                    ScriptBootstrapAction(path=MonitoringConfigurationSpark3().IMPORT_TTD_CERTS_PATH, args=[])
                ]
